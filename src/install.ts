...
import { classifyPnpmFailure, HOST_NAMESPACE_RE } from './pnpm-compat.ts'
 ...
 * an error naming neither plugin, from which the pluginhub's own page is
 ...
 * whole profile, not just this plugin, refuses to start, with the pluginhub's
 ...
 * ids to resolve, so one row per owner is the unit the pluginhub renders and
...
export function parsePrepareNotAllowed(stdout: string, stderr: string): string | null {
  // The pluginhub always runs pnpm with --reporter=ndjson, so this sentence
