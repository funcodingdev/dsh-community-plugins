 ...
 *
 * The pluginhub ALSO keeps its own in-memory/state.json bookkeeping (hot-mount
 ...
 *   worse, the pluginhub reports it instead;
...
/** Row ids the pluginhub is allowed to write: plain unquoted YAML scalars. */
const ROW_ID_RE = /^[A-Za-z0-9_.-]+$/u

/**
 * Line-wise scan of one patch file — the plugin-hub shapes. Deliberately
 * not a YAML parse: the file may hold structures the pluginhub's dialect
 ...
 * package's own bundle declaration: uninstall may clean pluginhub-owned rows,
 ...
 * Empty for client-only packages (no bundle rows) — the pluginhub's own
 * state.json mechanism covers those, and there is nothing to patch.
 * PluginHub-owned namespaces (hot-mount `pluginhub-*`, shim `client-*`) are
 * excluded: their rows live in the pluginhub's own include subtree, and a
...
export function rowIdsForPackage(host: PatchHost, profileDirectory: string, packageName: string): string[] {
  ...
  for (const entry of host.loader.entries()) {
    ...
    if (/^(?:pluginhub-|client-)/u.test(id)) continue
    ids.add(id)
  }
  return [...ids]
}
 ...
 * dsh-postgres-backends disables session-persistence-jsonl, so once the pluginhub
 ...
 * the e2e fixture-cross tweaks dsph-fixture-b's config, and #147 requires
 ...
 * OUTSIDE the pluginhub (hand-edited cordis.patch.yml, dsh-web-plugin-manager,
 * the dsh CLI), which the pluginhub's own state.json never sees.
 */
 ...
 * the pluginhub reports it and keeps its own live/state.json toggle instead.
 */
...
/** Remove every disable/force block the pluginhub (or the user) wrote for a
