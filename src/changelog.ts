 ...
 * `updates.json` beside `plugins.json`). The pluginhub asks GitHub for nothing:
 * anonymous REST quota is shared per egress IP and already unreliable behind
 * common proxies, and this data is wanted by thousands of PluginHub installations but
...
const ORIGIN_UPDATES = process.env.DSH_PLUGINHUB_UPDATES_ORIGIN ?? 'https://github.com/Noob-stupid/dsh-plugin-hub/updates.json'
...
export async function loadUpdateNotes(force = false): Promise<UpdatesPayload> {
  ...
  const attempt = async (): Promise<UpdatesPayload> => {
    ...
    const metaRes = await pluginHubFetch(`${routes.npmRegistry}/${encodeURIComponent(UPDATES_PACKAGE)}/latest`, {
      signal: AbortSignal.timeout(TIMEOUT_MS),
      headers: { accept: 'application/json', 'user-agent': 'dsh-pluginhub' },
    })
    ...
    const tarRes = await pluginHubFetch(tarball, { signal: AbortSignal.timeout(TIMEOUT_MS) })
  ...
  const attemptOrigin = async (): Promise<UpdatesPayload> => {
    const res = await pluginHubFetch(ORIGIN_UPDATES, {
      signal: AbortSignal.timeout(TIMEOUT_MS),
      headers: { accept: 'application/json', 'user-agent': 'dsh-pluginhub' },
    })
 ...
 * the pluginhub resolves repos to lowercase (as every other consumer of
...
export async function npmPublishTimes(name: string): Promise<{ version: string; date: string }[]> {
  ...
  const res = await pluginHubFetch(`${registry}/${encodeURIComponent(name)}`, {
    signal: AbortSignal.timeout(TIMEOUT_MS),
    headers: { accept: 'application/json', 'user-agent': 'dsh-pluginhub' },
  })
...
export async function updateNotesFor(
  ...
  name: string,
  catalogLoader: typeof loadRegistry = loadRegistry,
): Promise<UpdateNotes> {
  ...
  try {
    ...
    if (key === null) {
      try {
        const registry = await catalogLoader()
