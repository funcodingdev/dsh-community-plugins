/**
 * The pluginhub's card on the plugin configuration page (dsh >= 0.1.0-rc.7).
 *
 * It manages the pluginhub ITSELF — version, update, remove — when the current
 * profile owns the dependency. A host-provided pluginhub keeps only its version
 ...
 * mirror it one for one, so the pluginhub stops looking like it wandered in
...
import { api, setGithubProxy } from './pluginhub-data.ts'
...
export interface SettingsCardProps {
  t: Translate
  /**
   * Retire the pluginhub's own entry in the left settings menu.
   *
   * Called once the package is gone. Leaving "社区插件" in the menu after
...
/** What api('/dsh-pluginhub/status') tells this card. */
interface SelfStatus {
  ...
  regionAuto: boolean
  /** Whether the profile package manager owns this pluginhub installation. */
  selfManaged: boolean
}

/** The subset of api('/dsh-pluginhub/status') this card reads. */
...
/** What api('/dsh-pluginhub/updates') says about the pluginhub's own row. */
...
/** The pluginhub's own row as api('/dsh-pluginhub/updates') sends it. */
...
export function SettingsCard({ t, onRemoved }: SettingsCardProps): ReactElement | null {
   ...
   * (#39). Only the pluginhub's own card can update the pluginhub, so without a
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const response = await fetch(api('/dsh-pluginhub/status'), { cache: 'no-store' })
      ...
      try {
        const response = await fetch(api('/dsh-pluginhub/updates'), { cache: 'no-store' })
        const body = (await response.json()) as { updates?: Record<string, RawUpdate> }
        const own = body.updates?.[PLUGINHUB_PACKAGE_NAME]
  ...
  const onUpdate = useCallback((force = false) => {
    ...
    void (async () => {
      try {
        const body = await post(api('/dsh-pluginhub/update'), {
  ...
  const onRemove = useCallback(() => {
    ...
    void (async () => {
      try {
        const body = await post(api('/dsh-pluginhub/self-uninstall'), { confirm: true, purge })
        if (body.ok === true) {
          setPhase('removed')
          onRemoved?.()
  ...
  const refreshUpdate = useCallback(async (): Promise<void> => {
    const response = await fetch(api('/dsh-pluginhub/updates') + '?force=1', { cache: 'no-store' })
    const body = (await response.json()) as { updates?: Record<string, RawUpdate> }
    const own = body.updates?.[PLUGINHUB_PACKAGE_NAME]
  ...
  const onChannel = useCallback((next: Channel) => {
    ...
    void (async () => {
      try {
        const body = await post(api('/dsh-pluginhub/channel'), { channel: next }) as { ok?: boolean; error?: string; channel?: string }
   ...
   *
   * A hand-made choice retires the one-time notice: the pluginhub no longer has
  ...
  const onRegion = useCallback((next: Region) => {
    ...
    void (async () => {
      try {
        const body = await post(api('/dsh-pluginhub/region'), { region: next }) as { ok?: boolean; error?: string; region?: string }
        if (body.ok !== true) { setError(body.error ?? t('setSelfFailed')); return }
        ...
        try {
          const fresh = await fetch(api('/dsh-pluginhub/status'), { cache: 'no-store' })
  ...
  const body = phase === 'removed'
    ...
    : h(Fragment, null,
        ...
        // getting plugins at all, which is the pluginhub's whole job, while the
        // channel is about which build of the pluginhub itself arrives.
        row(
          t('setRegion'),
        ...
        status?.selfManaged === true && (phase === 'confirming' || busy)
          ? h('div', { className: css.setConfirm },
              ...
              // pluginhub switched off stay off, with the UI that could switch
        ...
        // The one place the pluginhub can be forced past pnpm's fresh-release
