 ...
 * persisted as the decision. Re-probing every boot would let a pluginhub
 ...
 *
 * The pluginhub's own package: present on every npm mirror worth using, and
 ...
 */
const PROBE_PATH = `${encodeURIComponent(PLUGINHUB_PACKAGE_NAME)}/latest`
...
export async function probeRegion(timeoutMs = 2500, env: NodeJS.ProcessEnv = process.env): Promise<Region> {
  ...
  const ask = async (region: Region): Promise<Region> => {
    ...
    const res = await pluginHubFetch(`${base}/${PROBE_PATH}`, {
