 ...
 * network, no writes. It answers, for the profile the pluginhub is serving:
 *
...
export interface PeerMismatch {
  ...
  optional?: boolean
  /**
   * The directional verdict for rows that did not match. Host-side safety
   * checks compute it because compatibility tiering needs metadata from disk
   * and cannot be guessed from the range string (#201).
   */
  verdict?: PeerVerdict
}
 ...
 * directory contents — safe to call on every pluginhub open.
 */
export function analyzeProfile(profileDirectory: string, options: CheckOptions = {}): CheckReport {
  ...
  for (const mismatch of peerMismatches) {
    ...
    // for the pluginhub's own optional peer, on every profile that installs us
    // (#275 by @tjxjxjx).
