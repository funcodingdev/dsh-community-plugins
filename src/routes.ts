/**
 * HTTP routes bridging the browser pluginhub UI to the host. This layer only
 ...
 * orchestration in install.ts / live-toggle.ts / updates.ts.
 *
...
import { createRegistryClient } from './registry.ts'
import {
  cleanHotDir, hotMount, hotUnmount, listHotMounts, MAX_NOTE,
  mountClientOnlyDeps, purgePluginHubState, readPluginHubState, writePluginHubState,
} from './hot.ts'
import { createGroup, deleteGroup, removeFromGroups, renameGroup, setGroupMembers } from './groups.ts'
import { configurePersistentLog, logEvent } from './log.ts'
import { pluginHubFetch } from './net.ts'
...
import { readBundleStack } from './order.ts'
...
import { setEntryDisabled, type LoaderEntry } from './live-toggle.ts'
import { readJsonBody, sameOrigin, sendJson } from './http.ts'
import { detectedDebugger, detectedSupervisor, restartAllowed, scheduleRestart, servingPort, trustedRestartRequest } from './restart.ts'
...
export type { LoaderEntry } from './live-toggle.ts'
...
export interface PluginHubHost extends Pick<Context, 'effect'> {
...
export interface PluginHubConfig {
  /** Profile the pluginhub installs into; matches the profile serving this UI. */
  profile: string
  ...
  /** Which release channel the pluginhub offers ITSELF from; other plugins never follow it. */
  channel?: Channel
...
/**
 * The pluginhub's own version, read once from its installed package.json.
 *
...
export function pluginHubVersion(): string {
...
/** The pluginhub's own package names, as they appear in a profile manifest. */
const SELF_NAMES = new Set<string>([PLUGINHUB_PACKAGE_NAME])
...
/**
 * Register the pluginhub's HTTP routes.
 * @param host - Scope providing webServer and loader services.
 * @param config - Validated pluginhub configuration.
 * @returns Disposer removing every registered route.
 */
export function mountPluginHubRoutes(
  host: PluginHubHost,
  config: PluginHubConfig,
  ...
  if (config.profileDirectory === undefined && !isDshProfileName(config.profile)) {
    ...
    // swallows it: the routes silently never mount and EVERY /dsh-pluginhub/*
    // request answers 404 with nothing anywhere saying why — the pluginhub
    ...
    const message = `dsh-pluginhub: invalid profile name ${JSON.stringify(config.profile)}; the pluginhub's routes were not mounted and every /dsh-pluginhub/* request will answer 404. Use the same non-empty, non-traversal profile name accepted by DSH, or pass an explicit profile directory.`
    host.logger?.warn(`[dsh-pluginhub] ${message}`)
    logEvent('error', 'mount', message)
    throw new Error(message)
  }
  const activeProfileDir = profileDir(config.profile, config.profileDirectory)
  configurePersistentLog(join(activeProfileDir, '.dsh-pluginhub', 'log.ndjson'))
  ...
  const pluginHubState = readPluginHubState(activeProfileDir)
  const disabled = pluginHubState.disabled
  const groups = pluginHubState.groups
  const groupOrder = pluginHubState.groupOrder
  // A choice made in a previous session outranks whatever the entry layer
  // composed, which is only ever a default.
  if (pluginHubState.channel !== undefined) config.channel = pluginHubState.channel
  const activeChannel = (): Channel => resolveChannel(config.channel, pluginHubVersion())
  ...
  // routed immediately.
  if (pluginHubState.region !== undefined) config.region = pluginHubState.region
  let region: Region = config.region ?? 'global'
  const catalog = createRegistryClient(() => region)
  const { loadRegistry, loadRegistryPlugin, forgetCatalog } = catalog
  let regionAuto = pluginHubState.regionAuto === true
  ...
  if (config.region === undefined) {
    void resolveRegion(undefined).then(({ region: probed }) => {
      ...
      applyRegion(probed)
      regionAuto = true
      // Persisted as the decision, not re-probed each boot: a pluginhub that
      // silently changes routes between runs makes "it was fast yesterday"
      // impossible to investigate.
      pluginHubState.region = probed
      pluginHubState.regionAuto = true
      config.region = probed
      writePluginHubState(activeProfileDir, pluginHubState)
   ...
   * reference (live handlers) sees the fresh state (issue
   * #98 review M2).
   */
  function refreshPluginHubState(): void {
    const fresh = readPluginHubState(activeProfileDir)
    disabled.clear()
    ...
    // mutated in place. These three are read off `pluginHubState` itself and
    // were not being refreshed at all — which is #435: a note written
    // through this route reached disk, but `pluginHubState.notes` still held
    ...
    pluginHubState.notes = fresh.notes
    pluginHubState.channel = fresh.channel
    pluginHubState.region = fresh.region
    pluginHubState.regionAuto = fresh.regionAuto
  }

  // Client-only packages (dsh.client without dsh.bundle) are invisible to the
  // bundle layer in every boot; the pluginhub shim-mounts them so their client
  ...
  void mountClientOnlyDeps(host, activeProfileDir).then(async (mounted) => {
    ...
    for (const name of disabled) {
      if (disposed) return
      if (await setEntryDisabled(host, name, true)) logEvent('info', 'boot', `plugin kept off: ${name}`)
    }
  }).catch(error => {
    if (!disposed) logEvent('warn', 'boot', `plugin activation failed: ${String(error)}`)
  })
  ...
  host.on?.('internal/plugin', (fiber) => {
    ...
    if (name !== undefined && disabled.has(name)) void setEntryDisabled(host, name, true)
  })
  ...
  async function setPluginEnabled(name: string, enabled: boolean): Promise<{ ok: boolean; reason?: string }> {
    ...
    if (enabled) {
      if (listHotMounts().includes(name)) {
        ok = true
      } else if (await setEntryDisabled(host, name, false)) {
        ok = true
      } else {
      ...
      ok = await hotUnmount(name) || await setEntryDisabled(host, name, true)
      if (!ok) {
    ...
    writePluginHubState(dir, { disabled, groups, groupOrder })
    return { ok, reason }
  }
  ...
  /**
   * Drop live hot mounts whose package was removed outside the pluginhub
  ...
  async function dropStaleHotMounts(): Promise<void> {
    for (const name of listHotMounts()) {
      ...
      logEvent('warn', 'hot-sweep', `${name}: package removed outside the pluginhub — live mount dropped`)
    }
  }
  ...
  async function rollbackUpdateBuild(
    ...
    // CI=true (the pluginhub always runs pnpm that way) turns frozen-lockfile
  ...
  function restoreProfileLockfile(snapshot: ProfileLockfileSnapshot): { ok: boolean; detail: string | null } {
    ...
    const temp = `${file}.dsh-pluginhub-rollback-${process.pid}-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`
    try {
  ...
  async function removeInstalledPackage(name: string): Promise<{ ok: boolean; hot: boolean; detail: string | null }> {
    ...
    const entryDisabled = await setEntryDisabled(host, name, true)
    ...
    writePluginHubState(activeProfileDir, { disabled, groups, groupOrder })
   ...
   * rollback that leaves such a row behind is fixed, but the pluginhub issues
  ...
  async function staleSubpath(target: string): Promise<string | null> {
    ...
    try {
      const res = await pluginHubFetch(
  ...
  const disposers = [
    host.webServer.register({
      kind: 'exact',
      path: '/dsh-pluginhub/api/v1/capabilities',
      handler: (request, response) => {
        ...
        sendJson(response, 200, {
          ...
          restart: {
            ...
            managedBy: canRestart ? 'pluginhub' : config.profileDirectory === undefined ? 'operator' : 'desktop-host',
          ...
          endpoints: {
            updates: '/dsh-pluginhub/api/v1/updates',
            operations: '/dsh-pluginhub/api/v1/operations',
            rollback: '/dsh-pluginhub/api/v1/rollback',
            restart: '/dsh-pluginhub/api/v1/restart',
          },
        })
      },
    }),
    ...
    host.webServer.register({
      kind: 'exact',
      path: '/dsh-pluginhub/api/v1/updates',
      handler: async (request, response) => {
        ...
        try {
          ...
          void invokeLegacy('/dsh-pluginhub/update', request, 'POST', {
            name: packageName,
    ...
    host.webServer.register({
      kind: 'exact',
      path: '/dsh-pluginhub/api/v1/operations',
      handler: (request, response) => {
        if (request.method !== 'GET') {
          response.writeHead(405, { allow: 'GET' })
          response.end()
    ...
    host.webServer.register({
      kind: 'exact',
      path: '/dsh-pluginhub/api/v1/rollback',
      handler: async (request, response) => {
        if (request.method !== 'POST') {
        ...
        try {
          ...
          const result = await invokeLegacy('/dsh-pluginhub/rollback', request, 'POST', { rollbackId: legacyRollbackId })
    ...
    host.webServer.register({
      kind: 'exact',
      path: '/dsh-pluginhub/api/v1/restart',
      handler: async (request, response) => {
        if (request.method !== 'POST') {
          response.writeHead(405, { allow: 'POST' })
          response.end()
        ...
        const result = await invokeLegacy('/dsh-pluginhub/restart', request, 'POST', {})
      ...
      handler: async (request, response) => {
        ...
        try {
          try {
            ...
            sendJson(response, 200, { registry: await loadRegistry(region, { force }) })
          } catch (error) {
            // Say what went wrong. The pluginhub used to substitute a bundled
    ...
    host.webServer.register({
      ...
      handler: async (request, response) => {
        ...
        // show toggles made OUTSIDE the pluginhub (hand-edited cordis.patch.yml,
        ...
        sendJson(response, 200, {
          ...
          groups,
          groupOrder,
          notes: readPluginHubState(activeProfileDir).notes ?? {},
        ...
        try {
          await withMutationLock(response, 'write', async () => {
            ...
            if (SELF_NAMES.has(name)) {
              sendJson(response, 400, { error: 'the pluginhub cannot be disabled from its own page; use the dsh CLI' })
              return
            }
      ...
      handler: async (request, response) => {
        ...
        try {
          await withMutationLock(response, 'write', async () => {
            ...
            const state = readPluginHubState(activeProfileDir)
            ...
            writePluginHubState(activeProfileDir, { ...state, notes })
            refreshPluginHubState()
    ...
    host.webServer.register({
      ...
      handler: async (request, response) => {
        ...
        sendJson(response, 200, {
          ...
          supervisor: detectedSupervisor(),
          debugger: detectedDebugger(),
          selfManaged: installed[PLUGINHUB_PACKAGE_NAME] !== undefined,
          installed,
        })
      },
    }),
    ...
    host.webServer.register({
      ...
      handler: async (request, response) => {
        ...
        try {
          ...
          // Only the pluginhub itself follows the channel setting (see
          // PluginHubSettings.channel): a user opting into betas is volunteering
    ...
    host.webServer.register({
      ...
      handler: async (request, response) => {
        ...
        try {
          ...
          sendJson(response, 200, await updateNotesFor(config.profile, activeProfileDir, name, loadRegistry))
        } catch (error) {
          sendJson(response, 500, { error: error instanceof Error ? error.message : String(error) })
        }
      },
    ...
    host.webServer.register(captureLegacy('/dsh-pluginhub/update', {
      kind: 'exact',
      path: '/dsh-pluginhub/update',
      handler: async (request, response) => {
        if (request.method !== 'POST') {
          response.writeHead(405, { allow: 'POST' })
          response.end()
        ...
        try {
          await withMutationLock(response, 'install', async () => {
            ...
            if (restore && SELF_NAMES.has(name) && spec.toLowerCase().startsWith('link:')) {
              sendJson(response, 400, { error: 'PluginHub的本地开发链接不会被线上版本替换。 / The pluginhub\'s local development link is never replaced by an online release.' })
              return
            }
            if (isLocalSpec(spec)) {
              if (!restore) {
            ...
            const isNpmRollbackSource = !restore && !isGit && !isReleaseTarball
              // PluginHub-managed npm installs persist only a range, version, or
            ...
            // The pluginhub follows its channel; everything else is `latest`.
            ...
            const staleError = !stale
              ...
              : staleReason === 'release-age'
                ...
                : '更新命令执行完成，但版本没有变化，原因未能确认。点「立即更新」重试通常能解决；若仍不行，请将页面错误信息反馈到 issue。 / The update command completed but the version did not change; the cause could not be confirmed. Clicking "Update now" to retry usually resolves it — if not, include the on-screen error in your issue.'
          ...
          const message = error instanceof Error ? error.message : String(error)
          host.logger?.warn(`[dsh-pluginhub] update failed: ${message}`)
    ...
    /**
     * Remove the pluginhub itself, from its card on the plugin configuration
     * page. Deliberately NOT the generic uninstall route, which keeps
     * refusing the pluginhub: a destructive action on the thing serving the
     * request should be reachable only from the surface built for it, and
     * never as a stray `{ name: "dsh-pluginhub" }` on the ordinary path.
     *
     ...
     * normally. The profile boots clean afterwards, with the pluginhub's rows
     * gone from `dependencies` and `dsh.profile.bundles`.
     */
    /**
     * Which release channel the pluginhub offers ITSELF from.
     *
    ...
    host.webServer.register({
      ...
      handler: async (request, response) => {
        ...
        try {
          ...
          // Persisted with the pluginhub's own durable state, so the choice
          // survives a restart — a setting that forgets is a setting the
          // user has to make again every boot.
          pluginHubState.channel = wanted
          writePluginHubState(activeProfileDir, pluginHubState)
     ...
     * for the reason recorded there: a value the pluginhub stores in its own
     * state.json cannot also be owned by the settings schema without the two
     * writing over each other.
     */
    host.webServer.register({
      kind: 'exact',
      path: '/dsh-pluginhub/region',
      handler: async (request, response) => {
        if (request.method !== 'POST') {
          response.writeHead(405, { allow: 'POST' })
          response.end()
    ...
    host.webServer.register({
      kind: 'exact',
      path: '/dsh-pluginhub/self-uninstall',
      handler: async (request, response) => {
        if (request.method !== 'POST') {
          response.writeHead(405, { allow: 'POST' })
          response.end()
        ...
        // end the pluginhub's life in this process, so neither may be driven by
        ...
        try {
          await withMutationLock(response, 'install', async () => {
            ...
            const installed = readInstalled(config.profile, activeProfileDir)
            const selfName = installed[PLUGINHUB_PACKAGE_NAME] !== undefined ? PLUGINHUB_PACKAGE_NAME : undefined
            if (selfName === undefined) {
              sendJson(response, 400, { error: 'the pluginhub is not an installed dependency of this profile' })
              return
            }
            ...
            if (!ok) {
              ...
              sendJson(response, 502, {
                ok: false,
                error: said === '' ? 'removing the pluginhub failed' : said,
                timedOut: result.timedOut,
            ...
            // Opt-in cleanup. Rows the pluginhub wrote to the USER patch layer
            // outlive it: a plugin switched off here stays off after the
            // pluginhub is gone, and the only UI that could switch it back on
            // has just been removed. Only rows belonging to packages on the
            // pluginhub's own disable list are touched — a hand-written row is
            // the user's, not ours.
            ...
            if (purge) {
              ...
              purgePluginHubState(activeProfileDir)
            }
            ...
            // without the pluginhub instead. Deferred because it disposes the
            ...
            // stating it: the browser drops the pluginhub the moment this runs,
            // and the leftover disabled entry is cleared by whatever restart
            // happens next. There is no decision to offer.
            setTimeout(() => {
              void setEntryDisabled(host, selfName, true).catch(() => { /* a later restart resolves it either way */ })
            }, 0)
          })
        } catch (error) {
          sendJson(response, 500, { error: error instanceof Error ? error.message : String(error) })
    ...
    host.webServer.register(captureLegacy('/dsh-pluginhub/restart', {
      kind: 'exact',
      path: '/dsh-pluginhub/restart',
      handler: (request, response) => {
        if (request.method !== 'POST') {
          response.writeHead(405, { allow: 'POST' })
          response.end()
    ...
    host.webServer.register({
      kind: 'exact',
      path: '/dsh-pluginhub/approve-builds',
      handler: async (request, response) => {
        if (request.method !== 'POST') {
          response.writeHead(405, { allow: 'POST' })
          response.end()
      ...
      handler: async (request, response) => {
        ...
        try {
          await withMutationLock(response, 'install', async () => {
            ...
            const force = body.force === true
            if (SELF_NAMES.has(name)) {
              sendJson(response, 400, { error: 'the pluginhub cannot uninstall itself; use the dsh CLI' })
              return
            }
            if (readInstalled(config.profile, activeProfileDir)[name] === undefined) {
              sendJson(response, 400, { error: 'plugin is not installed' })
            ...
            if (userPatchReferences === null && !force) {
              ...
              // refusing with NO way through is the wrong shape: the pluginhub
            ...
            if (userPatchReferences !== null && userPatchReferences.length > 0) {
              ...
              sendJson(response, 409, {
                error: `无法卸载 ${name}：当前 profile 的 cordis.patch.yml 仍通过 insert 引用 ${listed}。请先移除这些用户补丁引用再重试；PluginHub不会自动改写用户补丁。 / Cannot uninstall ${name}: this profile's cordis.patch.yml still inserts ${listed}. Remove those user-owned patch references first and retry; the pluginhub will not rewrite the user patch automatically.`,
          ...
          const message = error instanceof Error ? error.message : String(error)
          host.logger?.warn(`[dsh-pluginhub] uninstall failed: ${message}`)
    ...
    host.webServer.register(captureLegacy('/dsh-pluginhub/rollback', {
      kind: 'exact',
      path: '/dsh-pluginhub/rollback',
      handler: async (request, response) => {
        if (request.method !== 'POST') {
          response.writeHead(405, { allow: 'POST' })
          response.end()
        ...
        try {
          ...
          const message = error instanceof Error ? error.message : String(error)
          host.logger?.warn(`[dsh-pluginhub] rollback failed: ${message}`)
      ...
      handler: async (request, response) => {
        ...
        try {
          await withMutationLock(response, 'install', async () => {
            ...
            const entry = await loadRegistryPlugin(url)
            if (entry === undefined) {
            ...
            if (entry.installable === false) {
              const reason = entry.validationReason || entry.validationStatus || 'pluginhub validation failed'
              logEvent('warn', 'install-rejected', `${entry.name}: pluginhub marked the plugin ${entry.validationStatus ?? 'uninstallable'} (${reason})`)
              sendJson(response, 400, {
                error: `插件尚未通过 DSH 插件结构校验，暂不可安装：${reason} / this plugin has not passed DSH structure validation and cannot be installed yet: ${reason}`,
            ...
            sendJson(response, ok || cancelled ? 200 : result.busy === true ? 409 : 502, {
              ...
              error: conflictGroups.length > 0
                ...
                : addedNothing
                  ...
                  // is.
                  ? '安装命令报告成功，但 profile 没有任何变化——插件本身没问题，是执行安装的通道没有真正运行。若使用桌面端，请改用命令行 dsh plugin add 验证，并把页面错误信息附在 issue 中 / the install command reported success but the profile did not change — the plugin is not at fault, the channel that should have installed it did not actually run. On a desktop build, verify with `dsh plugin add` from a terminal and include the on-screen error in your issue'
                  : notAPlugin
                    ? 'nothing installable: the plugin(s) need a build step (blocked by default, see allowBuilds) or ship no prebuilt artifacts / 没有可安装的内容：插件需要构建授权（allowBuilds，默认拦截）或未附带构建产物'
          ...
          const message = error instanceof Error ? error.message : String(error)
          host.logger?.warn(`[dsh-pluginhub] install failed: ${message}`)
