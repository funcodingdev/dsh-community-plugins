 ...
 * composition through a pluginhub-owned Include subtree.
 *
 ...
 * its input files live under `<profile>/.dsh-pluginhub/` and are wiped on every
 ...
 * `state.json` in the same directory is the pluginhub's own durable state
 ...
 */
const HOT_MOUNT_TIMEOUT_MS = Number(process.env.DSH_PLUGINHUB_HOT_MOUNT_TIMEOUT_MS) || 10000
...
async function loadHotTreeClass(): Promise<unknown | null> {
  ...
  try {
    ...
    class PluginHubHotTree extends Include {
...
function readPkgDsh(profileDir: string, packageName: string): { client?: unknown; bundle?: { patch?: unknown } } | null {
  try {
    const manifest = JSON.parse(
      readFileSync(join(profileDir, 'node_modules', packageName, 'package.json'), 'utf8'),
    ) as { dsh?: { client?: unknown; bundle?: { patch?: unknown } } }
    return manifest.dsh ?? {}
  } catch {
    return null
  }
...
/**
 * Wipe leftover hot-mount inputs; call once when the pluginhub host starts.
...
/** Persisted pluginhub state: the generic disable list plus custom groups. */
export interface PluginHubState {
   ...
   * few fields they own and hand it to writePluginHubState. Requiring this one
   ...
   * it or the user picked it — no further probing happens, so the pluginhub
   ...
   *
   * Only drives a one-time notice explaining why the pluginhub picked what it
...
/**
 * Read the whole pluginhub state. Legacy `disabledSkins` (the pre-#60
...
export function readPluginHubState(profileDir: string): PluginHubState {
  try {
...
/**
 * Persist the whole pluginhub state.
 *
 ...
 * state — `writePluginHubState(dir, { disabled, groups, groupOrder })` appears
 ...
 * through a fresh read while the long-lived `pluginHubState` in routes.ts still
 ...
 */
export function writePluginHubState(profileDir: string, state: PluginHubState): void {
  mkdirSync(join(profileDir, HOT_DIR), { recursive: true, mode: 0o700 })
  const onDisk = readPluginHubState(profileDir)
  // `?? {}` only for the type: PluginHubState declares notes optional, while
  // readPluginHubState always returns an object.
...
export function readDisabled(profileDir: string): Set<string> {
  return readPluginHubState(profileDir).disabled
}
...
export function writeDisabled(profileDir: string, disabled: Set<string>): void {
  const state = readPluginHubState(profileDir)
  state.disabled = new Set(disabled)
  writePluginHubState(profileDir, state)
}

/** Package names currently live through a pluginhub hot mount (patch or shim). */
...
function raceActivationTimeout<T>(awaitable: T | Promise<T>): Promise<T> {
  return new Promise<T>((resolve, reject) => {
    const timer = setTimeout(() => {
      reject(new Error(`activation did not settle within ${HOT_MOUNT_TIMEOUT_MS / 1000}s — the plugin may be waiting on a service that never arrives`))
 ...
 * composition.
 * @param ctx - pluginhub host context; the subtree unwinds with the pluginhub's fiber.
...
export async function hotMount(ctx: HotContext, profileDir: string, packageName: string): Promise<HotMountResult> {
  ...
  try {
    ...
    if (typeof patch === 'string' && patch !== '') {
      const patchText = readFileSync(join(profileDir, 'node_modules', packageName, patch), 'utf8')
    ...
    hotSequence += 1
    const mountFile = join(dir, `hot-${String(hotSequence)}.yml`)
    file = mountFile
    const yml = rows
      .map(row => `- id: 'pluginhub-${row.id}'\n  name: '${row.name}'\n`)
      .join('')
    writeFileSync(file, yml)
    const fiber = ctx.plugin(HotTree, { path: pathToFileURL(file).href })
    handle = fiber
    let disposed = false
    const mounted: PluginHandle = {
      await: () => fiber.await(),
      dispose: async () => {
        try { await release?.() } finally { await fiber.dispose() }
      },
    }
    release = ctx.effect(() => () => {
      disposed = true
      if (hotHandles.get(packageName) === mounted) {
        hotHandles.delete(packageName)
        shimNames.delete(packageName)
      }
      rmSync(mountFile, { force: true })
    }, `dsh-pluginhub: hot mount ${packageName}`)
    await raceActivationTimeout(fiber.await())
    if (disposed) throw new Error('host scope unloaded during activation')
    hotHandles.set(packageName, mounted)
    ctx.logger?.info?.(`[dsh-pluginhub] hot-mounted ${packageName}`)
    logEvent('info', 'hot-mount', `${packageName}: live${shimNames.has(packageName) ? ' (client-only shim)' : ''}`)
    return { ok: true, reason: null }
  } catch (error) {
    // Failed activation must release the subtree as well as its metadata.
    // Disposal is best-effort so a wedged plugin cannot hold the route open.
    try { Promise.resolve(release?.()).catch(() => {}) } catch { /* best effort */ }
    ...
    ctx.logger?.warn(`[dsh-pluginhub] hot mount of ${packageName} failed, restart required: ${message}`)
 ...
 * `dsh.bundle`) at pluginhub startup. The bundle reconcile skips these packages
 * entirely, so without the pluginhub's shim their client bundles are unreachable
 ...
 * the pluginhub's strict patch parser rejects, but any mention of a row id or
...
/**
 * Delete the pluginhub's own state directory.
 *
 ...
 * are the user's durable choices. Uninstalling the pluginhub is the one moment
 * where removing them is the right thing, and only when the user asked.
 * @returns true when a directory was there to remove.
 */
export function purgePluginHubState(profileDir: string): boolean {
  const dir = join(profileDir, HOT_DIR)
  try {
