...
export interface UpdateStatus {
   ...
   * which was tried and leaked immediately: the pluginhub page has three
 ...
 * running pluginhub, not of any one question asked of it, and threading it
...
async function fetchJson(url: string): Promise<unknown> {
  ...
  const res = await pluginHubFetch(url, {
 ...
 * `latest` and the pluginhub can finally offer the way back. That case used to
...
export async function checkUpdates(
   ...
   * ever the pluginhub itself: opting into early builds is volunteering to try
   * THIS plugin early, not a licence to pull every other author's
   * unreleased work.
   */
  channelFor: ReadonlyMap<string, Channel> = new Map(),
  /**
   * Curated npm sources for `file:` installs that were matched to the pluginhub
