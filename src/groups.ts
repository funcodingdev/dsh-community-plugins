...
/** The slices of pluginhub state group CRUD touches (routes.ts owns the rest). */
 ...
 * collapse, so the persisted list stays clean.
 */
export function setGroupMembers(
  ...
  for (const member of members) {
    ...
    // The pluginhub itself never participates in groups (#60 assumptions).
    if (member === PLUGINHUB_PACKAGE_NAME) continue
    seen.add(member)
