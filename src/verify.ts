/**
 * Post-install activation verification: what "installed" actually
 ...
 *             client-only — the pluginhub shim-mounts those at boot)
 ...
 * package (`dsh-pluginhub`, `@scope/pkg`), but may point at a subpath entry
 ...
 *
 * Without this the pluginhub kept telling users to restart for a plugin that
 ...
 * pluginhub's hot-mount table (injectable for tests).
 */
/**
 ...
 * fell through to `broken` — the pluginhub told them the install had failed
...
export function verifyActivation(
  ...
  // the switch state (pluginhub disable list or the user patch layer) is the
  // dominant fact, and the loader keeps it off on every boot.
  if (isDisabled) {
    return {
      state: 'disabled',
      reasons: ['已停用(PluginHub开关或补丁层),重启后保持关闭 / disabled (pluginhub toggle or the patch layer) — stays off across restarts'],
      bundle: inBundles,
      hot: false,
    }
  }
  ...
  // (the dsh CLI skips them), so the pluginhub shim-mounts them at boot —
  // they still work, but "installed" never means "bundle layer".
  if (dsh.client !== undefined) {
    return {
      state: 'inert',
      reasons: [
        '未声明 dsh.bundle,不会进入 profile bundle 层(纯客户端插件);重启后由PluginHub自动挂载生效 / no dsh.bundle — client-only plugins never enter the bundle layer; the pluginhub shim-mounts them at the next boot',
      ],
      bundle: false,
      hot: false,
    }
 ...
 *
 * Measured on a real host rather than reasoned about — updating the pluginhub
 * from 1.11.3 to 1.12.2 left `/dsh-pluginhub/status` reporting 1.11.3 with an
 ...
 * no server code: the pluginhub shim-mounts it so the loader has a live row,
