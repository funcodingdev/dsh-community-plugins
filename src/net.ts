/**
 * Outbound HTTP for the pluginhub's own server-side calls.
 *
 ...
 * `CONNECT github.com/Noob-stupid/dsh-plugin-hub:443`.
 *
 * So the pluginhub calls undici's fetch with an explicit dispatcher. The scope
 ...
 * the agent explicitly in `pluginHubFetch`.
 *
 ...
 */
export async function pluginHubFetch(
  url: string,
