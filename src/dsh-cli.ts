 ...
 * pluginhub's children find their tools regardless of how dsh was started.
 */
/**
 ...
 * Every later spawn sees them, so the pluginhub does not have to be restarted
 ...
 * `net.ts` already routes the pluginhub's own catalog fetches through — but
 ...
 *
 * That gap is why the pluginhub could load its catalog through a proxy and
...
export function proxyEnvForPnpm(env: NodeJS.ProcessEnv = process.env, region: Region = 'global'): NodeJS.ProcessEnv {
  ...
  // Three consumers, three vocabularies, one proxy. The pluginhub's own fetch
 ...
 * manager lives in. The pluginhub appends the places it is actually installed
 * to rather than telling the user to fix their environment.
 *
 * Windows used to get only the Node directory, which made the pluginhub's own
...
const INSTALL_TIMEOUT_MS = Number(process.env.DSH_PLUGINHUB_INSTALL_TIMEOUT_MS) || 15 * 60 * 1000
...
export interface InstallResult {
   ...
   * stderr — which for a pluginhub install is dsh's wrapper line, "pnpm failed
 ...
 * below appear nowhere in `@deepseek-ai/*`. Every other client the pluginhub
 * runs under, including other desktop apps, installs through the ordinary
 * `dsh plugin --profile <p> add` CLI, and so does the pluginhub itself when
...
export interface DesktopPnpmLike {
   ...
   * instead, which their launcher enables only for the selected pluginhub
   ...
   * installed on that host by any spelling this pluginhub could send. That is
 ...
 * pluginhub sends a bare name for a registry plugin and `dsh-community-plugins@latest` for
...
const NPM_ONLY_HOST_NOTE
  = '这个桌面客户端只能安装已发布到 npm 的插件，而该插件仅提供 GitHub 源，因此装不了——这是客户端的安装边界，不是插件或PluginHub的问题。'
  ...
  + 'That is the client\'s install boundary, not a fault in the plugin or the pluginhub. '
...
/** The child of the operation currently running, for /dsh-pluginhub/cancel. */
...
function killTree(child: ChildProcess): void {
  ...
  // the close event never fires and the pluginhub looks stuck "installing".
 ...
 * collapsing both into `false` made the pluginhub give one answer to both: it
...
export async function provisionPnpm(): Promise<{ ok: boolean; hint?: string }> {
  ...
  // not look (#149: corepack exit=0, npm -g exit=0, and the pluginhub still
...
/**
 * Whether a bare command name resolves to a file on the PATH the pluginhub
 * hands its children.
 *
 * The pluginhub cannot read the reason a spawn failed out of the child's
 ...
 *
 * Every one of these was a real report where the pluginhub said only "自动准备
...
export function runDshPlugin(profile: string, pluginArgs: string[]): Promise<InstallResult> {
  ...
  if (viaShell && !isCmdSafeProfileName(profile)) {
    const error = `dsh-pluginhub: profile name ${JSON.stringify(profile)} cannot cross the Windows cmd.exe fallback safely; relaunch DSH through its Node entry point, or use a profile name containing only letters, numbers, spaces, dots, underscores, and hyphens`
 ...
 * pluginhub runner. There is no runtime import or dependency on Desktop: the
...
export function createDesktopPluginRuntime(
  ...
  if (!isAbsolute(activeProfileDir) || activeProfileDir.includes('\0')) {
    throw new Error('dsh-pluginhub: Desktop profile directory must be an absolute path without NUL')
  }
  if (!isAbsolute(invokingDir) || invokingDir.includes('\0')) {
    throw new Error('dsh-pluginhub: Desktop invoking directory must be an absolute path without NUL')
  }
  const owner = Symbol('dsh-pluginhub desktop runtime')
  let closed = false
  ...
  const runPlugin: PluginRunner = async (_profile, pluginArgs) => {
    if (closed) {
      return {
        ...
        timedOut: false,
        stdout: '',
        stderr: 'dsh-pluginhub: Desktop package runtime is disposed',
        cancelled: false,
      }
    }
    ...
    timer = setTimeout(() => {
      timedOut = true
      abort.abort(new Error('dsh-pluginhub: Desktop package operation timed out'))
