 ...
 *
 * The pluginhub UI has richer internal response shapes that evolve with its UI.
 * Third-party plugins must not depend on those shapes, so this module owns the
 * small JSON envelope exposed under `/dsh-pluginhub/api/v1`.
 */

import type { InstallProgress } from './dsh-cli.ts'

export const UPDATE_API_V1_SCHEMA = 'dsh-pluginhub/update-api/v1' as const
