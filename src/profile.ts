/**
 * Profile filesystem reads — everything the pluginhub learns from a dsh
...
export function profileDir(profile: string, explicitDir?: string): string {
  ...
  if (!isDshProfileName(profile)) throw new Error(`dsh-pluginhub: invalid profile name ${JSON.stringify(profile)}`)
  return join(resolveDshHome(), 'profiles', profile)
}

/**
 * The in-box bundles dsh's profile templates install themselves — the ONLY
 * names the pluginhub hides from the installed list. Community plugins may
 ...
 * source-only checkout the #18 guard removes — so the pluginhub both flagged it
...
export function hasLoadableEntry(profileDirectory: string, name: string): boolean {
  ...
  // exiting 0 was immediately followed by the pluginhub removing what it had
 ...
 * for every later pnpm run in the profile (and for the pluginhub itself).
