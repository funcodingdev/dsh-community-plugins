 ...
 *
 * This exists because the failure it prevents is the worst one the pluginhub
 ...
 * debug-attached host from the pluginhub UI is the right answer.
 *
...
export function restartHelperSource(
  ...
  return [
    ...
    'const note = (line) => { try { fs.appendFileSync(logErr, `[dsh-pluginhub] ${line}\n`) } catch {} }',
...
export function scheduleRestart(port: number | null = null): RestartResult {
  ...
  const logOut = join(tmpdir(), `dsh-pluginhub-restart-${stamp}.out.log`)
  const logErr = join(tmpdir(), `dsh-pluginhub-restart-${stamp}.err.log`)
