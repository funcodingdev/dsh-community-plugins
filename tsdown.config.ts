/**
 * Browser client bundle for the dsh-pluginhub plugin, mirroring the DeepSeek
 ...
 * exact `window.__ModuleLoader__.load({ id: <npm package name>` prefix.
 */
import { readFile } from 'node:fs/promises'
import { basename, dirname, relative, resolve as resolvePath } from 'node:path'
import { defineConfig } from 'tsdown'
import { transform } from 'lightningcss'
import { PLUGINHUB_PACKAGE_NAME } from './src/package-name.ts'

const id = PLUGINHUB_PACKAGE_NAME

/**
 * Externals resolved from the host's loader module table at runtime.
 */
...
export default defineConfig({
  ...
  deps: {
    ...
    // Shared host modules must stay external; only the icon library is bundled.
