...
> releases. `GET /dsh-pluginhub/api/v1/capabilities` reports
...
`dsh-pluginhub` exposes a small, versioned, same-origin JSON API for plugin-owned
update surfaces. It lets a plugin show its own update button without spawning a
package manager, copying the PluginHub installation algorithm, or binding to the
PluginHub UI's private response fields.
...
```http
GET /dsh-pluginhub/api/v1/capabilities
```

The response names the PluginHub version (`pluginHubVersion`), profile, runtime
(`web` or `desktop`), supported features, restart owner and endpoint paths.
`restart.managedBy` is `pluginhub` when PluginHub owns the restart. A client must hide its
...
```http
GET /dsh-pluginhub/api/v1/updates?name=dsh-mcp-connector&force=1
```

The response includes the installed version, target version, source kind and
whether the target is a forward update. Omitting `force=1` allows the PluginHub's
short update-check cache.

## Start and observe an update

Mutation requests require the same-origin protection used by the PluginHub UI.

```http
POST /dsh-pluginhub/api/v1/updates
...
```http
GET /dsh-pluginhub/api/v1/operations?operationId=<id>
```
...
For npm updates, PluginHub verifies the package version that pnpm actually placed
...
```http
POST /dsh-pluginhub/api/v1/rollback
Content-Type: application/json
...
only when the PluginHub's compatibility verification retained a recovery point;
...
```http
POST /dsh-pluginhub/api/v1/restart
Content-Type: application/json

{}
```

This preserves the PluginHub's stricter restart guard: direct loopback,
...
- When discovery is unavailable, plugin UIs should fall back to opening the
  PluginHub rather than calling legacy `/dsh-pluginhub/*` mutation routes directly.
