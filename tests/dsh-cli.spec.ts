...
describe('nodeExecutable (Android linker64 execPath)', () => {
  ...
  // `error: expected absolute path: "--expose-internals"` — every pluginhub
...
describe('proxy env translated for the pnpm subprocess (#148/#161/#188/#232/#274)', () => {
  // Three consumers, three vocabularies. The pluginhub's own fetch reads the
...
describe('TARGET_RE plugin target allowlist', () => {
  it('accepts semver range prefixes that restore/install flows produce', () => {
    ...
    for (const target of [
      ...
      'dsh-pluginhub@^1.14.1',
      'dsh-pluginhub@=1.2.3',
      'dsh-pluginhub@1.14.0',
...
describe('toolSearchDirs (#292)', () => {
  ...
  // pluginhub appends the places a package manager is actually installed to.
