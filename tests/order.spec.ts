...
let tmp: string
beforeEach(() => {
  tmp = mkdtempSync(join(tmpdir(), 'dsph-order-'))
...
describe('readBundleStack (#98 order)', () => {
  it('classifies in-box official bundles vs community bundles, keeping manifest order', () => {
    ...
    writeProfile(dir, {
      name: 'web-profile',
      dsh: {
        profile: { bundles: ['@deepseek-ai/dsh-base', 'dsh-pluginhub', '@deepseek-ai/dsh-web-app', 'demo-plugin'] },
      },
    })

    const stack = readBundleStack(dir)
    expect(stack.bundles).toEqual(['@deepseek-ai/dsh-base', 'dsh-pluginhub', '@deepseek-ai/dsh-web-app', 'demo-plugin'])
    // Only the three in-box bundles are official; everything else is reorderable.
    expect(stack.community).toEqual(['dsh-pluginhub', 'demo-plugin'])
  })
