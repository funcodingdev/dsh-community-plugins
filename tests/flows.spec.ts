 ...
 * install, update-check, update, enable/disable, uninstall — through the REAL
 ...
 * - registry.ts  → fixed curated registry
...
vi.mock('../src/hot.ts', () => ({
  ...
  writePluginHubState: (_dir: string, state: {
...
const REGISTRY = {
  ...
  plugins: [
    ...
    // The pluginhub's own entry: the release-channel specs need it installed,
    // because the channel applies to this package and no other.
    { name: 'dsh-community-plugins', owner: 'funcodingdev', url: 'https://github.com/funcodingdev/dsh-community-plugins', category: 'tool', npm: 'dsh-community-plugins', description: {}, install: '', added: '' },
...
const registryModule = vi.hoisted(() => ({
  loadRegistry: vi.fn(),
  loadRegistryPlugin: vi.fn(),
  forgetCatalog: vi.fn(),
}))
vi.mock('../src/registry.ts', async (importOriginal) => ({
  ...await importOriginal<typeof import('../src/registry.ts')>(),
  ...registryModule,
  createRegistryClient: () => ({ ...registryModule, dispose: vi.fn() }),
}))
registryModule.loadRegistry.mockImplementation(() => Promise.resolve(REGISTRY))
registryModule.loadRegistryPlugin.mockImplementation((url: string) => Promise.resolve(
  REGISTRY.plugins.find(plugin => plugin.url.toLowerCase() === url.toLowerCase()),
))
...
// ---------------------------------------------------------------- testbed
import { pluginHubVersion, mountPluginHubRoutes } from '../src/routes.ts'
...
function createTestbed(
  ...
  runtime?: Parameters<typeof mountPluginHubRoutes>[2],
  ...
  const dispose = mountPluginHubRoutes(host as never, { profile: 'web', region: 'global', ...config }, runtime, () => agents)
...
beforeEach(() => {
  home = mkdtempSync(join(tmpdir(), 'dsph-flow-'))
...
describe('host-provided profile and package-operation seams', () => {
  it('mounts ordinary routes for a dotted, Unicode, spaced DSH profile name (#260)', async () => {
    ...
    const installed = await bed.dispatch('GET', '/dsh-pluginhub/installed')
  ...
  it('uses the explicit profile directory and injected status/setup/cancel operations', async () => {
    ...
    const status = await bed.dispatch('GET', '/dsh-pluginhub/status')
    expect(status.json).toMatchObject({
      pnpm: true, restart: false, selfManaged: false, installed: { 'desktop-only': '1.0.0' },
    })
    writeFileSync(join(explicitDir, 'package.json'), '{"dependencies":{"desktop-only":"1.0.0","dsh-community-plugins":"1.26.0"}}')
    expect((await bed.dispatch('GET', '/dsh-pluginhub/status')).json.selfManaged).toBe(true)
    expect(probe).toHaveBeenCalledTimes(2)
    expect((await bed.dispatch('POST', '/dsh-pluginhub/setup-pnpm', {})).json.ok).toBe(true)
    expect(provision).toHaveBeenCalledOnce()
    expect((await bed.dispatch('POST', '/dsh-pluginhub/cancel', {})).status).toBe(200)
  ...
  it('maps a generation-wide Desktop package-operation gate to conflict', async () => {
    ...
    const result = await bed.dispatch('POST', '/dsh-pluginhub/install', { url: 'https://github.com/o/dsh-loop' })
  ...
  it('writes build approvals and git keys only in the host-authoritative Desktop profile', async () => {
    ...
    const approve = await bed.dispatch('POST', '/dsh-pluginhub/approve-builds', { packages: ['dsh-blue-whale'] })
  ...
  it('restores the previous Desktop pin when an update fails after a partial manifest write', async () => {
    ...
    const result = await bed.dispatch('POST', '/dsh-pluginhub/update', { name: 'dsh-loop' })
    expect(result.status).toBe(502)
    const desktopManifest = JSON.parse(readFileSync(join(explicitDir, 'package.json'), 'utf8'))
  ...
  it('applies the same-name different-repo guard to the host-authoritative Desktop profile', async () => {
    ...
    const result = await bed.dispatch('POST', '/dsh-pluginhub/install', { url: 'https://github.com/a2/dsh-usage-stats' })
...
describe('install flow', () => {
   ...
   * since moved. Probe that source after pnpm fails so the response points
   * to the stale registry entry rather than the user's environment. */
  it('says a subpath entry is stale rather than letting pnpm look like the user fault', async () => {
    ...
    try {
      ...
      const result = await bed.dispatch('POST', '/dsh-pluginhub/install', {
        url: 'https://github.com/m/mono/tree/main/packages/plug-a',
      })
      expect(result.status).toBe(502)
      expect(String(result.json.staleEntry)).toContain('packages/plug-a')
  ...
  it('installs a curated plugin end to end and reports it installed', async () => {
    ...
    const r = await bed.dispatch('POST', '/dsh-pluginhub/install', { url: 'https://github.com/o/dsh-loop' })
    ...
    const listed = await bed.dispatch('GET', '/dsh-pluginhub/installed')
  ...
  it('reports host contracts declared as normal dependencies without rejecting the plugin', async () => {
    ...
    const installed = await bed.dispatch('POST', '/dsh-pluginhub/install', {
    ...
    const listed = await bed.dispatch('GET', '/dsh-pluginhub/installed')
    expect(listed.json.diagnostics.schema).toBe('dsh-pluginhub/diagnostics/v1')
  ...
  it('refuses sources outside the curated registry and cross-origin posts', async () => {
    const outside = await bed.dispatch('POST', '/dsh-pluginhub/install', { url: 'https://github.com/evil/mal' })
    expect(outside.status).toBe(400)
    const cross = await bed.dispatch('POST', '/dsh-pluginhub/install', { url: 'https://github.com/o/dsh-loop' }, { crossOrigin: true })
    expect(cross.status).toBe(403)
  })

  it('refuses a plugin the public pluginhub marks as not installable', async () => {
    registryModule.loadRegistryPlugin.mockResolvedValueOnce({
      ...REGISTRY.plugins[0],
      installable: false,
      validationStatus: 'invalid',
      validationReason: 'dsh-bundle-v3:missing_package_manifest',
    })
    const result = await bed.dispatch('POST', '/dsh-pluginhub/install', {
      url: 'https://github.com/o/dsh-loop',
    })
  ...
  it('retries around a peer on an unpublished host package, and only when the profile never asked for it (#289)', async () => {
    ...
    const installed = await bed.dispatch('POST', '/dsh-pluginhub/install', { url: 'https://github.com/o/dsh-loop' })
    expect(installed.status).toBe(200)
  ...
  it('rolls back manifest residue when the add fails after pnpm wrote package.json (#65)', async () => {
    ...
    // the pluginhub retries around that one instead (#289).
   ...
   * but the pluginhub issues one call and the HOST owns both writes, so any
  ...
  it('retargets a collection repo to its contained plugins via #path: (#18)', async () => {
    ...
    const r = await bed.dispatch('POST', '/dsh-pluginhub/install', { url: 'https://github.com/o/skin-pack' })
    expect(r.status).toBe(200)
...
describe('update flow — no npm publishing required', () => {
  beforeEach(async () => {
    ...
    fake.npm['dsh-loop'] = { latest: '1.0.0', versions: { '1.0.0': { manifest: { dsh: {}, main: 'lib/index.js' }, artifacts: ['lib/index.js'] } } }
    await bed.dispatch('POST', '/dsh-pluginhub/install', { url: 'https://github.com/o/dsh-loop' })
  })
  ...
  it('flags the update and applies it', async () => {
    ...
    const updates = await bed.dispatch('GET', '/dsh-pluginhub/updates?force=1')
    expect(updates.json.updates['dsh-loop']).toMatchObject({ kind: 'npm', current: '1.0.0', latest: '1.2.0', updateAvailable: true })
    const r = await bed.dispatch('POST', '/dsh-pluginhub/update', { name: 'dsh-loop' })
    expect(r.status).toBe(200)
    expect(r.json.ok).toBe(true)
    ...
    //
    // Measured — the pluginhub updated from 1.11.3 to 1.12.2 with 1.12.2 on
    // disk, `/dsh-pluginhub/status` still reporting 1.11.3, an unchanged boot
  ...
  it('exposes a versioned capability and update-check contract for plugin-owned UIs', async () => {
    advanceNpmLatest('1.2.0')
    const capabilities = await bed.dispatch('GET', '/dsh-pluginhub/api/v1/capabilities')
    expect(capabilities.status).toBe(200)
    expect(capabilities.json).toMatchObject({
      schema: 'dsh-pluginhub/update-api/v1',
      apiVersion: 1,
      ...
      stability: 'beta',
      pluginHubVersion: pluginHubVersion(),
      profile: 'web',
      runtime: 'web',
      features: { check: true, update: true, progress: true, rollback: true, restart: true },
      restart: { supported: true, managedBy: 'pluginhub' },
      operationRetention: 'current-process',
      operationLimit: 50,
    })

    const check = await bed.dispatch('GET', '/dsh-pluginhub/api/v1/updates?name=dsh-loop&force=1')
  ...
  it('returns an operation id immediately and exposes progress until the update settles', async () => {
    ...
    const accepted = await bed.dispatch('POST', '/dsh-pluginhub/api/v1/updates', {
    ...
    const concurrent = await bed.dispatch('POST', '/dsh-pluginhub/api/v1/updates', {
      packageName: 'dsh-loop',
    })
    expect(concurrent.status).toBe(409)
    expect(concurrent.json.failure).toMatchObject({ code: 'OPERATION_BUSY', retryable: true })

    const during = await bed.dispatch('GET', `/dsh-pluginhub/api/v1/operations?operationId=${operationId}`)
    ...
    for (let attempt = 0; attempt < 30 && completed.json.operation.state === 'running'; attempt += 1) {
      ...
      completed = await bed.dispatch('GET', `/dsh-pluginhub/api/v1/operations?operationId=${operationId}`)
    }
  ...
  it('normalizes an agent guard refusal as a terminal operation failure', async () => {
    ...
    const accepted = await guarded.dispatch('POST', '/dsh-pluginhub/api/v1/updates', {
      packageName: 'dsh-loop',
    })
    expect(accepted.status).toBe(202)
    const operationId = String(accepted.json.operation.operationId)
    let completed = await guarded.dispatch('GET', `/dsh-pluginhub/api/v1/operations?operationId=${operationId}`)
    for (let attempt = 0; attempt < 30 && completed.json.operation.state === 'running'; attempt += 1) {
      await new Promise(resolvePromise => setTimeout(resolvePromise, 5))
      completed = await guarded.dispatch('GET', `/dsh-pluginhub/api/v1/operations?operationId=${operationId}`)
    }
    expect(completed.json.operation).toMatchObject({
      state: 'failed',
      installedVersion: '1.0.0',
  ...
  it('keeps compatibility rollback private while exposing operation-scoped rollback', async () => {
    ...
    const accepted = await bed.dispatch('POST', '/dsh-pluginhub/api/v1/updates', { packageName: 'dsh-loop' })
    const operationId = String(accepted.json.operation.operationId)
    let completed = await bed.dispatch('GET', `/dsh-pluginhub/api/v1/operations?operationId=${operationId}`)
    for (let attempt = 0; attempt < 30 && completed.json.operation.state === 'running'; attempt += 1) {
      await new Promise(resolvePromise => setTimeout(resolvePromise, 5))
      completed = await bed.dispatch('GET', `/dsh-pluginhub/api/v1/operations?operationId=${operationId}`)
    }
    ...
    const rolledBack = await bed.dispatch('POST', '/dsh-pluginhub/api/v1/rollback', { operationId })
  ...
  it('updates a mirror-installed plugin from GitHub, not from a same-named npm package', async () => {
    // The spelling older pluginhub versions wrote under a mirrored region is a
    ...
    await bed.dispatch('POST', '/dsh-pluginhub/install', { url: 'https://github.com/o/r' })
    ...
    fake.calls = []
    const updated = await bed.dispatch('POST', '/dsh-pluginhub/update', { name: 'plug-b' })
  ...
  it('keeps a github subpath while dropping revision selectors during update (#281)', async () => {
    ...
    const direct = await bed.dispatch('POST', '/dsh-pluginhub/update', { name: 'plug-a' })
    ...
    const refreshed = await bed.dispatch('POST', '/dsh-pluginhub/update', { name: 'plug-a' })
  ...
  it('keeps the branch an update was installed from, alongside the subpath (#446)', async () => {
    ...
    const refreshed = await bed.dispatch('POST', '/dsh-pluginhub/update', { name: 'plug-b' })
    expect(refreshed.status).toBe(200)
  ...
  it('does not offer a rollback that the real CLI cannot execute for a github subpath', async () => {
    ...
    expect((await bed.dispatch('POST', '/dsh-pluginhub/install', {
      url: 'https://github.com/m/mono/tree/main/packages/plug-a',
    })).status).toBe(200)
    ...
    const updated = await bed.dispatch('POST', '/dsh-pluginhub/update', { name: 'plug-a' })
  ...
  it('refuses an update while any agent is running, before pnpm is touched', async () => {
    ...
    const r = await busyBed.dispatch('POST', '/dsh-pluginhub/update', { name: 'dsh-loop' })
  ...
  it('refuses install and uninstall while any agent is running, before pnpm is touched', async () => {
    const callsBefore = fake.calls.length
    const defaultStatus = await bed.dispatch('GET', '/dsh-pluginhub/status')
    expect(defaultStatus.json.agentGuardAvailable).toBe(false)
    const busyBed = createTestbed({}, undefined, {
      list: () => [{ id: 'main', status: 'running' }],
    })
    const busyStatus = await busyBed.dispatch('GET', '/dsh-pluginhub/status')
    expect(busyStatus.json.agentGuardAvailable).toBe(true)
    const install = await busyBed.dispatch('POST', '/dsh-pluginhub/install', { url: 'https://github.com/o/dsh-loop' })
    expect(install.status).toBe(409)
    expect(install.json.agentsBusy).toBe(true)
    expect(install.json.runningAgents).toEqual(['main'])

    const uninstall = await busyBed.dispatch('POST', '/dsh-pluginhub/uninstall', { name: 'dsh-loop' })
  ...
  it('allows the same update when no agent reports running', async () => {
    ...
    const r = await idleBed.dispatch('POST', '/dsh-pluginhub/update', { name: 'dsh-loop' })
    expect(r.status).toBe(200)
    expect(r.json.ok).toBe(true)
  ...
  it('refuses an update whose new version has no entry artifact (#159)', async () => {
    ...
    // and the pluginhub said "updated". The next boot could not resolve the
  ...
  it('restores the captured GitHub commit when a successful update has no entry artifact', async () => {
    const OLD = 'a'.repeat(40)
    const NEW = 'b'.repeat(40)
    await bed.dispatch('POST', '/dsh-pluginhub/uninstall', { name: 'dsh-loop' })
  ...
  it('rolls a soft host-incompatible npm update back to exact bytes while preserving its range (#195)', async () => {
    ...
    const rollback = await bed.dispatch('POST', '/dsh-pluginhub/rollback', { rollbackId: r.json.compatibility.rollbackId })
  ...
  it('reports a failed soft-incompatible exact rollback without claiming success', async () => {
    ...
    const updated = await bed.dispatch('POST', '/dsh-pluginhub/update', { name: 'dsh-loop' })
    expect(updated.status).toBe(200)
    expect(updated.json.compatibility).toMatchObject({ code: 'soft-incompatible' })
    fake.failAddTargetOnce = { target: 'dsh-loop@1.0.0', stderr: 'ELIFECYCLE: exact rollback failed' }
    const callsBeforeRollback = fake.calls.length

    const rollback = await bed.dispatch('POST', '/dsh-pluginhub/rollback', { rollbackId: updated.json.compatibility.rollbackId })
  ...
  it('refuses an update rollback token after an out-of-band profile edit', async () => {
    ...
    // PluginHub's in-process mutation lock after the warning was shown. The
    ...
    writeFileSync(manifestPath, JSON.stringify(manifest))
    const callsBeforeRollback = fake.calls.length
    const rollback = await bed.dispatch('POST', '/dsh-pluginhub/rollback', { rollbackId })
  ...
  it('serializes a toggle behind an in-flight exact rollback', async () => {
    ...
    let release!: () => void
    fake.gate = new Promise<void>((resolvePromise) => { release = resolvePromise })
    const rollback = bed.dispatch('POST', '/dsh-pluginhub/rollback', { rollbackId })
    await new Promise(resolvePromise => setTimeout(resolvePromise, 20))

    const toggle = await bed.dispatch('POST', '/dsh-pluginhub/toggle', { name: 'dsh-loop', enabled: false })
  ...
  it('does not offer exact rollback for a replaceable release-archive URL', async () => {
    ...
    const callsBefore = fake.calls.length
    const updated = await bed.dispatch('POST', '/dsh-pluginhub/update', { name: 'dsh-prebuilt' })
    expect(updated.status).toBe(200)
    expect(updated.json.compatibility).toMatchObject({ code: 'soft-incompatible' })
    expect(updated.json.compatibility.rollbackId).toBeUndefined()
  ...
  it('flags a soft host-incompatible install and rolls back the newly added plugin (#195)', async () => {
    ...
    const rollback = await bed.dispatch('POST', '/dsh-pluginhub/rollback', { rollbackId: install.json.compatibility.rollbackId })
    expect(rollback.status).toBe(200)
    expect(rollback.json.rolledBack).toBe(true)
    expect(installedSpec('dsh-loop')).toBeUndefined()
  ...
  it('surfaces the silent fresh-release hold as an actionable error, and force applies it (#22)', async () => {
    ...
    fake.staleUpdates = false
    const forced = await bed.dispatch('POST', '/dsh-pluginhub/update', { name: 'dsh-loop', force: true })
...
describe('local-dev restore flow', () => {
  it('refuses a plain update on a link: spec, and restore:true swaps it to the catalog', async () => {
    ...
    const blocked = await bed.dispatch('POST', '/dsh-pluginhub/update', { name: 'dsh-loop' })
    expect(blocked.status).toBe(400)
    expect(String(blocked.json.error)).toMatch(/locally linked/)
    expect(installedSpec('dsh-loop')).toBe('link:../dsh-loop-dev')

    const restored = await bed.dispatch('POST', '/dsh-pluginhub/update', { name: 'dsh-loop', restore: true })
  ...
  it('keeps #path: when restoring a monorepo checkout onto a collection-root catalog row', async () => {
    ...
    await bed.dispatch('POST', '/dsh-pluginhub/update', { name: 'theme-a', restore: true })
  ...
  it('refuses restore when the checkout still uses workspace: dependencies', async () => {
    ...
    const r = await bed.dispatch('POST', '/dsh-pluginhub/update', { name: 'theme-a', restore: true })
    expect(r.status).toBe(400)
  ...
  it('returns 400 when restore cannot find a catalog entry', async () => {
    ...
    const r = await bed.dispatch('POST', '/dsh-pluginhub/update', { name: 'mystery-plug', restore: true })
    expect(r.status).toBe(400)
  ...
  it('restores onto a prebuilt Release tarball without gluing a dist-tag to the URL', async () => {
    ...
    const r = await bed.dispatch('POST', '/dsh-pluginhub/update', { name: 'dsh-prebuilt', restore: true })
  ...
  it('refuses restore:true when the installed spec is not local', async () => {
    ...
    const r = await bed.dispatch('POST', '/dsh-pluginhub/update', { name: 'dsh-loop', restore: true })
    expect(r.status).toBe(400)
    expect(String(r.json.error)).toMatch(/Restore only applies/)
    expect(installedSpec('dsh-loop')).toBe('^1.0.0')
  })

  it('keeps the pluginhub development link local', async () => {
    writeFileSync(join(fake.profileDir, 'package.json'), JSON.stringify({
      dependencies: { 'dsh-community-plugins': 'link:../dsh-pluginhub-dev' },
    }))
    const r = await bed.dispatch('POST', '/dsh-pluginhub/update', { name: 'dsh-community-plugins', restore: true })
    expect(r.status).toBe(400)
    expect(String(r.json.error)).toMatch(/local development link/)
    expect(installedSpec('dsh-community-plugins')).toBe('link:../dsh-pluginhub-dev')
  })
  ...
  it('rolls a #path: restore back to the local spec when the catalog build introduces risks', async () => {
    ...
    const restored = await bed.dispatch('POST', '/dsh-pluginhub/update', { name: 'theme-a', restore: true })
    expect(restored.status, String(restored.json.error ?? '')).toBe(200)
    ...
    const rollback = await bed.dispatch('POST', '/dsh-pluginhub/rollback', { rollbackId: restored.json.compatibility.rollbackId })
    expect(rollback.status).toBe(200)
    expect(rollback.json.rolledBack).toBe(true)
    expect(installedSpec('theme-a')).toBe(`link:${checkout}`)
  })
...
describe('uninstall flow', () => {
  it('removes the plugin (live when hot mounted) and protects the pluginhub itself', async () => {
    fake.npm['dsh-loop'] = { latest: '1.0.0', versions: { '1.0.0': { manifest: { dsh: {}, main: 'lib/index.js' }, artifacts: ['lib/index.js'] } } }
    await bed.dispatch('POST', '/dsh-pluginhub/install', { url: 'https://github.com/o/dsh-loop' })
    const r = await bed.dispatch('POST', '/dsh-pluginhub/uninstall', { name: 'dsh-loop' })
    expect(r.status).toBe(200)
    expect(r.json.hot).toBe(true)
    expect(installedSpec('dsh-loop')).toBeUndefined()
    expect(hot.mounts).toEqual([])

    expect((await bed.dispatch('POST', '/dsh-pluginhub/uninstall', { name: 'dsh-community-plugins' })).status).toBe(400)
    expect((await bed.dispatch('POST', '/dsh-pluginhub/uninstall', { name: 'ghost' })).status).toBe(400)
  })
  ...
  it('refuses to uninstall when the user patch cannot be inspected safely (#165)', async () => {
    ...
    expect(readFileSync(patch, 'utf8')).toBe(patchText)
    expect(fake.calls.slice(callsBefore).some(call => call[0] === 'remove')).toBe(false)
    // Refusing is right, but refusing with no way through is not: the pluginhub
  ...
  it('lets an unreadable user patch be forced past, but never a definite reference (#165)', async () => {
    ...
    const forced = await bed.dispatch('POST', '/dsh-pluginhub/uninstall', { name: 'dsh-loop', force: true })
    ...
    const refused = await bed.dispatch('POST', '/dsh-pluginhub/uninstall', { name: 'dsh-loop', force: true })
  ...
  it('reconciles the manifest when a remove fails halfway (half-uninstall)', async () => {
    ...
    // activate the ghost dependency. The pluginhub must finish the removal.
...
describe('duplicate alias guard (#27)', () => {
  it('refuses installing the same repo again under another catalog name', async () => {
    ...
    expect((await bed.dispatch('POST', '/dsh-pluginhub/install', { url: 'https://github.com/h/dsh-share' })).status).toBe(200)
    ...
    const dup = await bed.dispatch('POST', '/dsh-pluginhub/install', { url: 'https://github.com/h/dsh-share' })
  ...
  it('refuses a same-named plugin from a DIFFERENT repo with an honest name-conflict error (#66)', async () => {
    ...
    const first = await bed.dispatch('POST', '/dsh-pluginhub/install', { url: 'https://github.com/a1/dsh-usage-stats' })
    ...
    const second = await bed.dispatch('POST', '/dsh-pluginhub/install', { url: 'https://github.com/a2/dsh-usage-stats' })
  ...
  it('does NOT block sibling subpackages of one monorepo', async () => {
    ...
    expect((await bed.dispatch('POST', '/dsh-pluginhub/install', { url: 'https://github.com/m/mono/tree/main/packages/plug-a' })).status).toBe(200)
    const second = await bed.dispatch('POST', '/dsh-pluginhub/install', { url: 'https://github.com/m/mono/tree/main/packages/plug-b' })
...
describe('pluginhub self-update', () => {
  it('switches a locally packaged pluginhub to its newer online release', async () => {
    await bed.dispatch('POST', '/dsh-pluginhub/channel', { channel: 'stable' })
    fake.npm['dsh-community-plugins'] = {
      latest: '1.0.3',
      versions: { '1.0.3': { manifest: { dsh: {}, main: 'lib/index.js' }, artifacts: ['lib/index.js'] } },
    }
    await bed.dispatch('POST', '/dsh-pluginhub/install', { url: 'https://github.com/funcodingdev/dsh-community-plugins' })
    const manifestPath = join(fake.profileDir, 'package.json')
    const manifest = JSON.parse(readFileSync(manifestPath, 'utf8')) as { dependencies: Record<string, string> }
    manifest.dependencies['dsh-community-plugins'] = 'file:/packages/dsh-pluginhub-1.0.3.tgz'
    writeFileSync(manifestPath, JSON.stringify(manifest))
    const packagePath = join(fake.profileDir, 'node_modules', 'dsh-community-plugins', 'package.json')
    const installedPackage = JSON.parse(readFileSync(packagePath, 'utf8')) as Record<string, unknown>
    installedPackage.repository = { type: 'git', url: 'https://github.com/funcodingdev/dsh-community-plugins.git' }
    writeFileSync(packagePath, JSON.stringify(installedPackage))
    fake.npm['dsh-community-plugins'].latest = '1.2.3'
    fake.npm['dsh-community-plugins'].versions['1.2.3'] = {
    ...
    const updates = await bed.dispatch('GET', '/dsh-pluginhub/updates?force=1')
    expect(updates.json.updates['dsh-community-plugins']).toMatchObject({
      current: '1.0.3', latest: '1.2.3', updateAvailable: true, restoreRequired: true,
    })
    const result = await bed.dispatch('POST', '/dsh-pluginhub/update', { name: 'dsh-community-plugins', restore: true })
    expect(result.status, String(result.json.error ?? '')).toBe(200)
    expect(installedSpec('dsh-community-plugins')).toBe('^1.2.3')
  })

  it('the pluginhub updates itself through the same flow', async () => {
    ...
    await bed.dispatch('POST', '/dsh-pluginhub/channel', { channel: 'stable' })
    fake.npm['dsh-community-plugins'] = { latest: '1.0.3', versions: { '1.0.3': { manifest: { dsh: {}, main: 'lib/index.js' }, artifacts: ['lib/index.js'] } } }
    await bed.dispatch('POST', '/dsh-pluginhub/install', { url: 'https://github.com/funcodingdev/dsh-community-plugins' })
    fake.npm['dsh-community-plugins'].latest = '1.2.3'
    fake.npm['dsh-community-plugins'].versions['1.2.3'] = { manifest: { dsh: {}, main: 'lib/index.js' }, artifacts: ['lib/index.js'] }
    vi.stubGlobal('fetch', (url: string) => String(url).includes('registry.npmjs.org')
      ? Promise.resolve(new Response(JSON.stringify({ version: '1.2.3' }), { status: 200 }))
      : Promise.reject(new Error('unexpected fetch')))
    const updates = await bed.dispatch('GET', '/dsh-pluginhub/updates?force=1')
    expect(updates.json.updates['dsh-community-plugins'].updateAvailable).toBe(true)
    const r = await bed.dispatch('POST', '/dsh-pluginhub/update', { name: 'dsh-community-plugins' })
    expect(r.status).toBe(200)
    expect(installedSpec('dsh-community-plugins')).toBe('^1.2.3')
  })
})

describe('GitHub plugin update and uninstall', () => {
  beforeEach(async () => {
    fake.repos['github:o/theme-a'] = { name: 'theme-a', manifest: { dsh: {}, main: 'index.js' }, artifacts: ['index.js'] }
    await bed.dispatch('POST', '/dsh-pluginhub/install', { url: 'https://github.com/o/theme-a' })
  })

  it('updates a GitHub-installed plugin by re-resolving its repo', async () => {
    const r = await bed.dispatch('POST', '/dsh-pluginhub/update', { name: 'theme-a' })
    expect(r.status).toBe(200)
    expect(fake.calls[fake.calls.length - 1]).toContain('github:o/theme-a')
  })

  it('uninstalls an active plugin and clears its live mount', async () => {
    expect(hot.mounts).toEqual(['theme-a'])
    const r = await bed.dispatch('POST', '/dsh-pluginhub/uninstall', { name: 'theme-a' })
    expect(r.status).toBe(200)
    expect(hot.mounts).toEqual([])
...
describe('concurrency', () => {
  it('a second install while one is running is refused with 409', async () => {
    ...
    let release!: () => void
    fake.gate = new Promise<void>((resolvePromise) => { release = resolvePromise })
    const first = bed.dispatch('POST', '/dsh-pluginhub/install', { url: 'https://github.com/o/dsh-loop' })
    await new Promise(resolvePromise => setTimeout(resolvePromise, 20)) // let it enter the executor
    const second = await bed.dispatch('POST', '/dsh-pluginhub/install', { url: 'https://github.com/o/theme-a' })
  ...
  it('status reports the route-level operation lock as busy while an install is in flight (#91)', async () => {
    ...
    let release!: () => void
    fake.gate = new Promise<void>((resolvePromise) => { release = resolvePromise })
    const install = bed.dispatch('POST', '/dsh-pluginhub/install', { url: 'https://github.com/o/dsh-loop' })
    await new Promise(resolvePromise => setTimeout(resolvePromise, 20))
    ...
    const during = await bed.dispatch('GET', '/dsh-pluginhub/status')
    expect(during.json.busy).toBe(true)
    release()
    fake.gate = null
    await install
    const after = await bed.dispatch('GET', '/dsh-pluginhub/status')
...
describe('cancel flow (#6)', () => {
  it('cancelling a running install ends it quietly (200 + cancelled, no error)', async () => {
    ...
    const install = bed.dispatch('POST', '/dsh-pluginhub/install', { url: 'https://github.com/o/dsh-loop' })
    await new Promise(resolvePromise => setTimeout(resolvePromise, 20))
    const cancel = await bed.dispatch('POST', '/dsh-pluginhub/cancel', {})
  ...
  it('cancel with nothing running is a 400', async () => {
    expect((await bed.dispatch('POST', '/dsh-pluginhub/cancel', {})).status).toBe(400)
  })
})
...
describe('build-script approval flow (#6)', () => {
  it('surfaces ignored builds, approve-builds allows only installed packages, and the retry succeeds', async () => {
    ...
    const first = await bed.dispatch('POST', '/dsh-pluginhub/install', { url: 'https://github.com/o/dsh-loop' })
    expect(first.json.ignoredBuilds).toEqual(['dsh-loop'])

    // Approval writes allowBuilds into the profile's pnpm-workspace.yaml…
    const approve = await bed.dispatch('POST', '/dsh-pluginhub/approve-builds', { packages: ['dsh-loop', 'ghost-package'] })
    expect(approve.status).toBe(200)
  ...
  it('approves TRANSITIVE build deps — in node_modules but not in package.json (#56)', async () => {
    ...
    const approve = await bed.dispatch('POST', '/dsh-pluginhub/approve-builds', { packages: ['cloudflared', '../evil', 'ghost-package'] })
    expect(approve.status).toBe(200)
  ...
  it('writes both allowBuilds key forms, so pnpm below 11.21 can match one (#285)', async () => {
    ...
    const approve = await bed.dispatch('POST', '/dsh-pluginhub/approve-builds', { packages: ['plug-c'] })
    expect(approve.status).toBe(200)
    const yaml = readFileSync(join(profileDir('web'), 'pnpm-workspace.yaml'), 'utf8')
  ...
  it('uses a commit-pinned github spec for old-pnpm build approval without re-resolving HEAD (#385)', async () => {
    ...
    const approve = await bed.dispatch('POST', '/dsh-pluginhub/approve-builds', { packages: ['plug-pinned'] })
    expect(approve.status).toBe(200)
    const yaml = readFileSync(join(profileDir('web'), 'pnpm-workspace.yaml'), 'utf8')
  ...
  it('surfaces a git-prepare rejection and approves the not-yet-installed package via the curated registry (#68)', async () => {
    ...
    const first = await bed.dispatch('POST', '/dsh-pluginhub/install', { url: 'https://github.com/omdsh-dev/dsh-security-audit' })
    ...
    const approve = await bed.dispatch('POST', '/dsh-pluginhub/approve-builds', { packages: ['dsh-security-audit'] })
    expect(approve.status).toBe(200)
    ...
    const retry = await bed.dispatch('POST', '/dsh-pluginhub/install', { url: 'https://github.com/omdsh-dev/dsh-security-audit' })
  ...
  it('writes the stable git allowBuilds key for an installed github-sourced dependency (#69)', async () => {
    ...
    await bed.dispatch('POST', '/dsh-pluginhub/install', { url: 'https://github.com/o/blue-whale' })
...
describe('official-scope community plugins (#28)', () => {
  it('installs and lists a community plugin named under @deepseek-ai/', async () => {
    ...
    const r = await bed.dispatch('POST', '/dsh-pluginhub/install', { url: 'https://github.com/omdsh-dev/dsh-security-audit' })
    expect(r.status).toBe(200)
    expect(r.json.ok).toBe(true)
...
describe('externally removed hot mounts (#29)', () => {
  it('drops a live mount whose package was removed outside the pluginhub', async () => {
    fake.npm['dsh-loop'] = { latest: '1.0.0', versions: { '1.0.0': { manifest: { dsh: {}, main: 'lib/index.js' }, artifacts: ['lib/index.js'] } } }
    await bed.dispatch('POST', '/dsh-pluginhub/install', { url: 'https://github.com/o/dsh-loop' })
    expect(hot.mounts).toEqual(['dsh-loop'])
    // Simulate `dsh plugin remove` outside the pluginhub: dep + files gone,
...
describe('one-click restart guards (#14)', () => {
  it('delegates the public v1 restart route to the guarded restart executor', async () => {
    const result = await bed.dispatch('POST', '/dsh-pluginhub/api/v1/restart', {})
  ...
  it('schedules exactly once for a trusted loopback request; repeat is 409', async () => {
    const r = await bed.dispatch('POST', '/dsh-pluginhub/restart', {})
    expect(r.status).toBe(202)
    expect(r.json.ok).toBe(true)
    expect(restartCalls.count).toBe(1)
    expect((await bed.dispatch('POST', '/dsh-pluginhub/restart', {})).status).toBe(409)
    expect(restartCalls.count).toBe(1)
  })

  it('refuses non-loopback peers, forwarded requests, and cross-origin posts', async () => {
    expect((await bed.dispatch('POST', '/dsh-pluginhub/restart', {}, { remoteAddress: '192.168.1.7' })).status).toBe(403)
    expect((await bed.dispatch('POST', '/dsh-pluginhub/restart', {}, { forwarded: true })).status).toBe(403)
    expect((await bed.dispatch('POST', '/dsh-pluginhub/restart', {}, { crossOrigin: true })).status).toBe(403)
  ...
  it('allowRestart: false disables the endpoint and the status capability flag', async () => {
    ...
    expect((await bed.dispatch('GET', '/dsh-pluginhub/status')).json.restart).toBe(false)
    expect((await bed.dispatch('POST', '/dsh-pluginhub/restart', {})).status).toBe(403)
    expect(restartCalls.count).toBe(0)
  })
  ...
  it('allowRestart: true does not override the debugger latch (#447)', async () => {
    ...
    debuggerLatch.value = 'inspector'
    expect((await bed.dispatch('GET', '/dsh-pluginhub/status')).json.restart).toBe(true)
    expect((await bed.dispatch('POST', '/dsh-pluginhub/restart', {})).status).toBe(403)
    expect(restartCalls.count).toBe(0)
  })
})
...
describe('bundle-layer uninstall live-disable (#37)', () => {
  it('uninstalling a bundle-layer plugin disables its live loader entry so refresh survives', async () => {
    ...
    const r = await bed.dispatch('POST', '/dsh-pluginhub/uninstall', { name: 'dsh-blue-whale' })
    expect(r.status).toBe(200)
...
describe('generic enable/disable toggle (#60)', () => {
  function installNpm(name: string, dsh: Record<string, unknown> = {}): Promise<void> {
    ...
    return bed.dispatch('POST', '/dsh-pluginhub/install', { url: `https://github.com/o/${name}` }).then(() => undefined)
  }
  ...
  it('toggles a hot-mounted plugin off and back on, persisting the disable list', async () => {
    ...
    const on = await bed.dispatch('POST', '/dsh-pluginhub/toggle', { name: 'dsh-loop', enabled: true })
  ...
  it('toggles a bundle-layer entry through setEntryDisabled', async () => {
    ...
    const off = await bed.dispatch('POST', '/dsh-pluginhub/toggle', { name: 'dsh-blue-whale', enabled: false })
    expect(off.status).toBe(200)
    expect(entry.options.disabled).toBe(true)
    expect(entry.fiber).toBeUndefined()
    expect(hot.disabled.has('dsh-blue-whale')).toBe(true)

    const on = await bed.dispatch('POST', '/dsh-pluginhub/toggle', { name: 'dsh-blue-whale', enabled: true })
    expect(on.status).toBe(200)
  ...
  it('writes the user patch layer on toggle (port of dsh-plugin-hub); activation reads disabled', async () => {
    ...
    await bed.dispatch('POST', '/dsh-pluginhub/install', { url: 'https://github.com/o/dsh-patchy' })
    hot.mounts = []
    ...
    const off = await bed.dispatch('POST', '/dsh-pluginhub/toggle', { name: 'dsh-patchy', enabled: false })
    expect(off.status).toBe(200)
    ...
    const on = await bed.dispatch('POST', '/dsh-pluginhub/toggle', { name: 'dsh-patchy', enabled: true })
    expect(on.status).toBe(200)
  ...
  it('reports restart + the reason when enabling cannot hot-mount', async () => {
    await installNpm('dsh-loop')
    await bed.dispatch('POST', '/dsh-pluginhub/toggle', { name: 'dsh-loop', enabled: false })
  ...
  it('rejects the pluginhub itself, unknown plugins, and cross-origin toggles', async () => {
    expect((await bed.dispatch('POST', '/dsh-pluginhub/toggle', { name: 'dsh-community-plugins', enabled: false })).status).toBe(400)
    expect((await bed.dispatch('POST', '/dsh-pluginhub/toggle', { name: 'ghost', enabled: true })).status).toBe(400)
    expect((await bed.dispatch('POST', '/dsh-pluginhub/toggle', { name: 'dsh-loop', enabled: false }, { crossOrigin: true })).status).toBe(403)
  })
  ...
  it('uninstall clears the disable flag; a reinstall starts enabled', async () => {
    ...
    await bed.dispatch('POST', '/dsh-pluginhub/toggle', { name: 'dsh-loop', enabled: false })
    expect(hot.disabled.has('dsh-loop')).toBe(true)
    const uninstall = await bed.dispatch('POST', '/dsh-pluginhub/uninstall', { name: 'dsh-loop' })
    expect(uninstall.status).toBe(200)
    expect(hot.disabled.has('dsh-loop')).toBe(false)
    const reinstall = await bed.dispatch('POST', '/dsh-pluginhub/install', { url: 'https://github.com/o/dsh-loop' })
...
describe('custom groups (#60)', () => {
  async function seedMembers(): Promise<void> {
    ...
    await bed.dispatch('POST', '/dsh-pluginhub/install', { url: 'https://github.com/o/dsh-loop' })
    await bed.dispatch('POST', '/dsh-pluginhub/install', { url: 'https://github.com/h/dsh-share' })
  }

  it('create/rename/delete lifecycle keeps groups and groupOrder consistent', async () => {
    const created = await bed.dispatch('POST', '/dsh-pluginhub/groups', { action: 'create', name: 'work' })
    expect(created.status).toBe(200)
    expect(created.json.groups).toEqual({ work: [] })
    expect(created.json.groupOrder).toEqual(['work'])

    expect((await bed.dispatch('POST', '/dsh-pluginhub/groups', { action: 'create', name: 'work' })).status).toBe(400)
    expect((await bed.dispatch('POST', '/dsh-pluginhub/groups', { action: 'create', name: '../evil' })).status).toBe(400)

    const renamed = await bed.dispatch('POST', '/dsh-pluginhub/groups', { action: 'rename', name: 'work', newName: 'daily' })
    expect(renamed.status).toBe(200)
    expect(renamed.json.groups).toEqual({ daily: [] })
    expect(renamed.json.groupOrder).toEqual(['daily'])

    const deleted = await bed.dispatch('POST', '/dsh-pluginhub/groups', { action: 'delete', name: 'daily' })
    expect(deleted.status).toBe(200)
    expect(deleted.json.groups).toEqual({})
    expect(deleted.json.groupOrder).toEqual([])
    expect((await bed.dispatch('POST', '/dsh-pluginhub/groups', { action: 'delete', name: 'ghost' })).status).toBe(400)
    expect((await bed.dispatch('POST', '/dsh-pluginhub/groups', { action: 'explode' })).status).toBe(400)
  })
  ...
  it('set-members keeps only installed plugins and uninstall prunes membership', async () => {
    ...
    const set = await bed.dispatch('POST', '/dsh-pluginhub/groups', {
      action: 'set-members', name: 'work', members: ['dsh-loop', 'dsh-share', 'ghost', 'dsh-community-plugins'],
    })
    expect(set.status).toBe(200)
    expect(set.json.groups.work.sort()).toEqual(['dsh-loop', 'dsh-share'])
    expect(set.json.groups.work).not.toContain('dsh-community-plugins')
  ...
  it('group toggle enables/disables every member as a batch', async () => {
    await seedMembers()
    await bed.dispatch('POST', '/dsh-pluginhub/groups', { action: 'create', name: 'work' })
    await bed.dispatch('POST', '/dsh-pluginhub/groups', { action: 'set-members', name: 'work', members: ['dsh-loop', 'dsh-share'] })
...
describe('self-uninstall — the pluginhub removing itself from its settings card', () => {
  /**
   * Deliberately a separate route from `/dsh-pluginhub/uninstall`, which keeps
   * refusing the pluginhub. A destructive action on the plugin serving the
   * request should be reachable only from the surface built for it, never as
   * a stray `{ name: "dsh-community-plugins" }` on the ordinary path.
   */
  it('is refused without an explicit confirmation', async () => {
    ...
    const bare = await bed.dispatch('POST', '/dsh-pluginhub/self-uninstall', {})
    expect(bare.status).toBe(400)
    expect(String(bare.json.error)).toContain('explicit confirmation')
    // `confirm: false` is not "close enough" either.
    const explicitlyNot = await bed.dispatch('POST', '/dsh-pluginhub/self-uninstall', { confirm: false })
  ...
  it('is refused from a cross-origin, forwarded or remote client', async () => {
    // The same door the restart route uses: both end the pluginhub's life in
    // this process, so neither may be driven by anything but the user's own
    // loopback browser.
    expect((await bed.dispatch('POST', '/dsh-pluginhub/self-uninstall', { confirm: true }, { crossOrigin: true })).status).toBe(403)
    expect((await bed.dispatch('POST', '/dsh-pluginhub/self-uninstall', { confirm: true }, { forwarded: true })).status).toBe(403)
    expect((await bed.dispatch('POST', '/dsh-pluginhub/self-uninstall', { confirm: true }, { remoteAddress: '192.168.1.7' })).status).toBe(403)
  })

  it('the ordinary uninstall route still refuses the pluginhub', async () => {
    // Adding a way to remove the pluginhub must not quietly open the old one.
    const viaGeneric = await bed.dispatch('POST', '/dsh-pluginhub/uninstall', { name: 'dsh-community-plugins' })
    expect(viaGeneric.status).toBe(400)
  })

  it('removes the pluginhub package only after confirmation and keeps state by default', async () => {
    fake.npm['dsh-community-plugins'] = {
      latest: '0.1.0',
      versions: { '0.1.0': { manifest: { dsh: {}, main: 'lib/index.js' }, artifacts: ['lib/index.js'] } },
    }
    const installed = await bed.dispatch('POST', '/dsh-pluginhub/install', { url: 'https://github.com/funcodingdev/dsh-community-plugins' })
    expect(installed.status).toBe(200)
    expect((await bed.dispatch('GET', '/dsh-pluginhub/status')).json.selfManaged).toBe(true)

    const removed = await bed.dispatch('POST', '/dsh-pluginhub/self-uninstall', { confirm: true })
    expect(removed.status).toBe(200)
    expect(removed.json).toMatchObject({ ok: true, removed: 'dsh-community-plugins', purged: false })
    expect(fake.calls).toContainEqual(['remove', 'dsh-community-plugins'])
    expect(installedSpec('dsh-community-plugins')).toBeUndefined()
    expect((await bed.dispatch('GET', '/dsh-pluginhub/status')).json.selfManaged).toBe(false)
    // Let the deferred loader disable complete before the testbed is disposed.
...
describe('download region', () => {
  it('does not let a late automatic probe replace a manual region choice', async () => {
    ...
    expect((await bed.dispatch('POST', '/dsh-pluginhub/region', { region: 'global' })).status).toBe(200)
    finishProbe({ region: 'china', probed: true })
    await new Promise(resolve => setTimeout(resolve, 0))

    expect((await bed.dispatch('GET', '/dsh-pluginhub/status')).json.region).toBe('global')
  ...
  it('does not let a disposed mount apply its late automatic probe', async () => {
    ...
    expect((await bed.dispatch('POST', '/dsh-pluginhub/region', { region: 'global' })).status).toBe(200)
    expect((await bed.dispatch('POST', '/dsh-pluginhub/note', {
  ...
  it('rejects anything but the two regions', async () => {
    expect((await bed.dispatch('POST', '/dsh-pluginhub/region', { region: 'CN' })).status).toBe(400)
    expect((await bed.dispatch('POST', '/dsh-pluginhub/region', {})).status).toBe(400)
    expect((await bed.dispatch('POST', '/dsh-pluginhub/region', { region: 'china' }, { crossOrigin: true })).status).toBe(403)
  })

  it('round-trips the setting and reports it on /status', async () => {
    const set = await bed.dispatch('POST', '/dsh-pluginhub/region', { region: 'china' })
    expect(set.status).toBe(200)
    expect(set.json.region).toBe('china')
    const status = (await bed.dispatch('GET', '/dsh-pluginhub/status')).json
    ...
    const back = await bed.dispatch('POST', '/dsh-pluginhub/region', { region: 'global' })
  ...
  it('sends the browser a resolved proxy prefix rather than a region to interpret', async () => {
    ...
    expect((await bed.dispatch('GET', '/dsh-pluginhub/status')).json.githubProxy).toBeNull()
    await bed.dispatch('POST', '/dsh-pluginhub/region', { region: 'china' })
    const proxy = (await bed.dispatch('GET', '/dsh-pluginhub/status')).json.githubProxy
    expect(typeof proxy).toBe('string')
    expect(String(proxy).startsWith('https://')).toBe(true)
    await bed.dispatch('POST', '/dsh-pluginhub/region', { region: 'global' })
  })

  it('stops offering the automatic explanation once the user has chosen', async () => {
    await bed.dispatch('POST', '/dsh-pluginhub/region', { region: 'china' })
    // The pluginhub has nothing left to explain: the answer is the user's now.
    expect((await bed.dispatch('GET', '/dsh-pluginhub/status')).json.regionAuto).toBe(false)
    await bed.dispatch('POST', '/dsh-pluginhub/region', { region: 'global' })
  })
})
...
describe('release channel', () => {
  it('rejects anything but the two channels', async () => {
    expect((await bed.dispatch('POST', '/dsh-pluginhub/channel', { channel: 'nightly' })).status).toBe(400)
    expect((await bed.dispatch('POST', '/dsh-pluginhub/channel', {})).status).toBe(400)
    expect((await bed.dispatch('POST', '/dsh-pluginhub/channel', { channel: 'beta' }, { crossOrigin: true })).status).toBe(403)
  })
  ...
  it('round-trips the setting', async () => {
    ...
    const set = await bed.dispatch('POST', '/dsh-pluginhub/channel', { channel: 'beta' })
    expect(set.status).toBe(200)
    expect(set.json.channel).toBe('beta')
    expect((await bed.dispatch('GET', '/dsh-pluginhub/status')).json.channel).toBe('beta')

    const back = await bed.dispatch('POST', '/dsh-pluginhub/channel', { channel: 'stable' })
    expect(back.status).toBe(200)
  ...
  it('installs from the channel it offered from, not from latest', async () => {
    ...
    fake.npm['dsh-community-plugins'] = { latest: '1.0.0', versions: { '1.0.0': { manifest: { dsh: {}, main: 'lib/index.js' }, artifacts: ['lib/index.js'] } } }
    await bed.dispatch('POST', '/dsh-pluginhub/install', { url: 'https://github.com/funcodingdev/dsh-community-plugins' })
    fake.npm['dsh-community-plugins'].latest = '9.0.0'
    fake.npm['dsh-community-plugins'].versions['9.0.0'] = { manifest: { dsh: {}, main: 'lib/index.js' }, artifacts: ['lib/index.js'] }
    await bed.dispatch('POST', '/dsh-pluginhub/channel', { channel: 'beta' })
    fake.calls = []
    await bed.dispatch('POST', '/dsh-pluginhub/update', { name: 'dsh-community-plugins' })
    const added = fake.calls.find(call => call[0] === 'add')
    expect(added?.join(' '), 'the update ran with the wrong dist-tag').toContain('dsh-community-plugins@beta')

    await bed.dispatch('POST', '/dsh-pluginhub/channel', { channel: 'stable' })
    fake.calls = []
    await bed.dispatch('POST', '/dsh-pluginhub/update', { name: 'dsh-community-plugins' })
    expect(fake.calls.find(call => call[0] === 'add')?.join(' ')).toContain('dsh-community-plugins@latest')
  })
  ...
  it('re-checks immediately when the channel changes', async () => {
    ...
    await bed.dispatch('POST', '/dsh-pluginhub/install', { url: 'https://github.com/funcodingdev/dsh-community-plugins' })
    const seen: string[] = []
    // Keep this assertion independent of the machine running the suite. When
    // a proxy env var is present pluginHubFetch correctly uses undici directly,
    // which would bypass the global fetch recorder below.
    const proxyKeys = ['http_proxy', 'HTTP_PROXY', 'https_proxy', 'HTTPS_PROXY', 'npm_config_proxy', 'npm_config_https_proxy'] as const
    ...
    try {
      ...
      await bed.dispatch('GET', '/dsh-pluginhub/updates')
      await bed.dispatch('POST', '/dsh-pluginhub/channel', { channel: 'beta' })
      seen.length = 0
      await bed.dispatch('GET', '/dsh-pluginhub/updates')
...
describe('catalog: one source, and a failure says so', () => {
  it('reports the reason instead of substituting a bundled copy', async () => {
    // A remote fetch failure must remain visible instead of supplying bundled data.
    registryModule.loadRegistry.mockRejectedValueOnce(new Error('fetch failed: ENOTFOUND'))
    const failed = await bed.dispatch('GET', '/dsh-pluginhub/registry')
...
describe('the channel choice survives a restart', () => {
  it('is written down, not just held in memory', async () => {
    ...
    expect((await bed.dispatch('POST', '/dsh-pluginhub/channel', { channel: 'beta' })).status).toBe(200)
    expect(hot.channel, 'the choice never reached durable state').toBe('beta')
  })

  it('is read back by a freshly mounted pluginhub', async () => {
    // 'stable' is the load-bearing direction, and deliberately so: this
    // build is a prerelease (1.14.0-beta.1), so a pluginhub that persisted
    ...
    expect((await bed.dispatch('POST', '/dsh-pluginhub/channel', { channel: 'stable' })).status).toBe(200)

    // A second pluginhub over the same profile state: this is what a restart
    // looks like from the state file's point of view.
    const restarted = createTestbed({ profile: 'web' })
    try {
      expect((await restarted.dispatch('GET', '/dsh-pluginhub/status')).json.channel).toBe('stable')
  ...
  it('leaves the channel derived from the build until the user picks one', async () => {
    ...
    // pluginHubVersion())` rather than a hardcoded 'beta' — a literal was only
    ...
    // dropping pluginHubVersion() entirely — both derive 'stable' here too, the
    ...
    try {
      expect(hot.channel).toBeUndefined()
      const expected = resolveChannel(undefined, pluginHubVersion())
      expect((await fresh.dispatch('GET', '/dsh-pluginhub/status')).json.channel).toBe(expected)
...
describe('the dev channel is an ordinary choice', () => {
  it('is offered by the status route alongside the other two', async () => {
    expect((await bed.dispatch('GET', '/dsh-pluginhub/status')).json.channels).toEqual(['stable', 'beta', 'dev'])
  })
  ...
  it('is selectable, persisted and read back like any other', async () => {
    ...
    expect((await bed.dispatch('POST', '/dsh-pluginhub/channel', { channel: 'dev' })).status).toBe(200)
    expect(hot.channel).toBe('dev')

    const restarted = createTestbed({ profile: 'web' })
    try {
      expect((await restarted.dispatch('GET', '/dsh-pluginhub/status')).json.channel).toBe('dev')
    } finally { restarted.dispose() }
  })

  it('still refuses a channel that does not exist', async () => {
    const refused = await bed.dispatch('POST', '/dsh-pluginhub/channel', { channel: 'nightly' })
    expect(refused.status).toBe(400)
    expect(hot.channel).toBeUndefined()
  })

  it('still refuses a cross-origin selection', async () => {
    expect((await bed.dispatch('POST', '/dsh-pluginhub/channel', { channel: 'dev' }, { crossOrigin: true })).status).toBe(403)
    expect(hot.channel).toBeUndefined()
  })
})
