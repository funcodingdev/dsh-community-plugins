...
vi.mock('../src/routes.ts', () => ({
  mountPluginHubRoutes: (host: unknown, config: Record<string, unknown>, runtime?: unknown) => {
...
describe('host adaptation', () => {
  it.each([['--profile', 'team'], ['--profile=team']])('uses the CLI profile from %j', (...args) => {
    const argv = process.argv
    process.argv = [process.execPath, 'dsh', ...args]
    try {
      apply(new FakeContext({ webServer: {}, loader: {} }) as never)
      expect(state.mounts[0].config.profile).toBe('team')
    } finally {
      process.argv = argv
    }
  })

  it('does not interpret application arguments after -- as a profile selector', () => {
    const argv = process.argv
    process.argv = [process.execPath, 'dsh', 'web', '--', '--profile=other']
    try {
      apply(new FakeContext({ webServer: {}, loader: {} }) as never)
