...
beforeEach(() => {
  tmp = mkdtempSync(join(tmpdir(), 'dsph-check-'))
...
describe('bundle stack (#98 diagnostics)', () => {
  it('keeps dsh.profile.bundles order and classifies official vs community', () => {
    ...
    writeProfile(dir, {
      name: 'web-profile',
      dsh: { profile: { bundles: ['@deepseek-ai/dsh-base', 'dsh-pluginhub'] } },
      dependencies: { '@deepseek-ai/dsh-base': '^4.0.1', 'dsh-pluginhub': '^1.9.0' },
    })
    writeBundle(dir, '@deepseek-ai/dsh-base', '4.0.1', [
      { insert: [{ id: 'dsh-base', name: 'dsh-base' }] },
    ])
    writeBundle(dir, 'dsh-pluginhub', '1.9.0', [
      { insert: [{ id: 'dsh-pluginhub', name: 'dsh-pluginhub' }] },
    ])
    ...
    expect(report.bundles.map(b => b.name)).toEqual(['@deepseek-ai/dsh-base', 'dsh-pluginhub'])
    ...
    expect(report.bundles[1]?.entries).toEqual(['dsh-pluginhub'])
    expect(report.rows.map(r => r.id)).toEqual(['dsh-base', 'dsh-pluginhub'])
...
describe('peer range mismatch', () => {
  ...
  it('does not WARN about an optional peer that does not match (#275)', () => {
    ...
    // including the pluginhub's own optional peer, on every profile that
    // installs it.
    const dir = pdir()
    writeProfile(dir, { name: 'web-profile', dependencies: {} })
