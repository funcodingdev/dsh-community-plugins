...
describe('updateNotesFor', () => {
  ...
  beforeEach(() => {
    ...
    forgetCatalog()
    vi.unstubAllGlobals()
    dir = mkdtempSync(join(tmpdir(), 'dsph-notes-'))
  })
  ...
  it('falls back to catalog lookup when spec is an npm package name', async () => {
    ...
    vi.stubGlobal('fetch', vi.fn(async (input: string | URL) => {
      ...
      // Catalog fetch — return the public pluginhub protocol with a
      // matching package name and semantic category.
      if (url.includes('dshpluginhub.com/plugins.json')) {
        return new Response(JSON.stringify({
          name: 'dsh-plugin-hub',
          ...
          count: 1,
          categories: {
            all: { en: 'All', zh: '全部' }, interface: { en: 'Interface', zh: '界面扩展' },
            development: { en: 'Developer tools', zh: '开发工具' }, automation: { en: 'Automation', zh: '自动化' },
            knowledge: { en: 'Knowledge & search', zh: '知识与检索' }, agent: { en: 'Agent capability', zh: 'Agent 能力' },
          },
          sorts: {
            recommended: { en: 'Recommended', zh: '推荐' }, updated: { en: 'Recently updated', zh: '最近更新' }, stars: { en: 'Most Stars', zh: '最多 Stars' },
          },
          plugins: [{
            name: 'npm-plugin', owner: 'owner', npm: '@scope/my-plugin', tarball: null,
            url: 'https://github.com/owner/repo', category: 'development', description: { en: 'plugin' },
            stars: 1, downloads: null, install: 'dsh plugin add @scope/my-plugin', added: '2026-09-03',
            updatedAt: '2026-09-03T00:00:00Z', isVerified: true, installable: true, validationStatus: 'verified',
            validationReason: '', requiresBuildAuthorization: false,
          }],
          pagination: { page: 1, pageSize: 48, total: 1, totalPages: 1, hasMore: false },
        }), { status: 200 })
      }
      return new Response('{}', { status: 200 })
    }))
