...
describe('pluginArgsFor', () => {
  ...
  function profileFixture(workspace: boolean): string {
    dir = mkdtempSync(join(tmpdir(), 'dsph-profile-'))
  ...
  it('injects -w exactly when the profile is a workspace root (#20)', () => {
    ...
    expect(pluginArgsFor(ws, ['add', 'dsh-pluginhub'])).toEqual(['add', '-w', 'dsh-pluginhub'])
    expect(pluginArgsFor(ws, ['remove', 'dsh-pluginhub'])).toEqual(['remove', '-w', 'dsh-pluginhub'])
    ...
    expect(pluginArgsFor(plain, ['add', 'dsh-pluginhub'])).toEqual(['add', 'dsh-pluginhub'])
    expect(pluginArgsFor(plain, ['remove', 'dsh-pluginhub'])).toEqual(['remove', 'dsh-pluginhub'])
  })
})
...
describe('classifyPnpmFailure', () => {
  ...
  it('explains a Windows locked-file rename instead of showing pnpm\'s stack (#389)', () => {
    // Reproduced from @qq1054435284's report: updating a plugin the
...
so pnpm cannot verify the downloaded tarball.`)
    ...
    expect(failed?.message).toContain('PluginHub不会自动为未经验证的字节生成校验值')
  ...
  it('names the violators in pnpm 11’s whole-lockfile verification report (#422)', () => {
    ...
    // an older pluginhub had written with a mirror-prefixed codeload URL. pnpm
...
pnpm now wants to use the store at "C:\\Users\\lenovo\\AppData\\Local\\pnpm\\store\\v11" to link dependencies.`
  ...
  it('is NOT marked recoverable — the pluginhub must not relink a whole node_modules on a guess', () => {
