/**
 * A real npm registry, served from disk, plus the pluginhub's curated catalog.
 *
 ...
 * tarball over localhost makes it exactly that — the pluginhub, pnpm and cordis
 ...
 * the dependency tree (the pluginhub's own deps) when it verifies the lockfile.
 */
...
export function packFixture(dir: string, destination: string): ServedPackage {
  ...
  // that cannot be spawned without a shell — the same trap the pluginhub's own
...
export function catalogEntry(pkg: ServedPackage): RegistryPlugin {
  return {
    ...
    url: `https://github.com/dsph-e2e/${pkg.name}`,
    npm: pkg.name,
    category: 'testing',
...
export interface FixtureRegistry {
  ...
  npmUrl: string
  /** Value for DSH_PLUGINHUB_REGISTRY_URL — the pluginhub's curated catalog. */
  catalogUrl: string
  close(): Promise<void>
}
...
export async function startFixtureRegistry(packages: ServedPackage[]): Promise<FixtureRegistry> {
  ...
  const server: Server = createServer((request, response) => {
    ...
    // The public pluginhub protocol installs from GitHub; this custom
    // catalog keeps npm fixture targets so the install chain stays local.
    if (url.pathname === '/fixture-catalog.json') {
  ...
  return {
    npmUrl: `${base}/`,
    catalogUrl: `${base}/fixture-catalog.json`,
