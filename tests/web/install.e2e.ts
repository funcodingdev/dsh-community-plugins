 ...
 * package, loaded the module and ran it. The pluginhub's own activation verdict
 * is an inference and is asserted AGAINST that, never trusted as it.
 *
 * No browser: the UI journey is covered by pluginhub.e2e.ts. What is
...
import { dshAvailable, exchangeProcessLaunchToken, launchPluginHubScaffold } from './scaffold.ts'
import type { WebScaffold } from './scaffold.ts'

const HAS_DSH = dshAvailable()
const A = 'dsph-e2e-fixture-a'
const B = 'dsph-e2e-fixture-b'
const CLASH = 'dsph-e2e-fixture-clash'
const CROSS = 'dsph-e2e-fixture-cross'
const CARRIER = 'dsph-e2e-fixture-carrier'
...
describe.skipIf(!HAS_DSH).sequential('web e2e: the real install chain', () => {
  ...
  beforeAll(async () => {
    scaffold = await launchPluginHubScaffold({ fixtures: ['fixture-a', 'fixture-b', 'fixture-clash', 'fixture-cross', 'fixture-carrier'] })
  ...
  const state = async (): Promise<InstalledState> =>
    (await fetch(`${base}/dsh-pluginhub/installed`)).json() as never
  ...
  const settle = async (): Promise<void> => {
    for (let attempt = 0; attempt < 90; attempt++) {
      const status = (await (await fetch(`${base}/dsh-pluginhub/status`)).json()) as { busy?: boolean; active?: boolean }
  ...
  /** Install through the pluginhub's own route and report the outcome. */
  const install = async (name: string): Promise<{ status: number; body: string }> => {
    const response = await post('/dsh-pluginhub/install', { url: `https://github.com/dsph-e2e/${name}` })
  ...
  it('installs through the route and cordis really mounts it', async () => {
    ...
    //
    // Carry the pluginhub's own verdict into the message: a bare "expected
  ...
  it('the pluginhub\'s own verdict matches that reality (#135)', async () => {
    const current = await state()
  ...
  it('refuses an install that would duplicate a loader entry id (#122)', async () => {
    // Two entries under one id make cordis refuse the NEXT boot — the pluginhub
  ...
  it('registers its own settings namespace on a host that has settings', async () => {
    ...
    expect(names, `served namespaces: ${names.join(', ')}`).toContain('dsh-pluginhub')
  }, 120_000)

  it('refuses a source that is not in the curated registry', async () => {
    const response = await post('/dsh-pluginhub/install', { url: 'https://github.com/attacker/not-listed' })
  ...
  it('a patch with config rows activates on restart, as the pluginhub says', async () => {
    const { status } = await install(CROSS)
    expect(status).toBe(200)
    // The pluginhub declines to hot-mount this shape and says so. Take it at
    ...
    // ...and the pluginhub has to AGREE once the restart it asked for happened
  ...
  it('disabling a plugin touches only the rows it owns (#147)', async () => {
    ...
    const response = await post('/dsh-pluginhub/toggle', { name: CROSS, enabled: false })
  ...
  it('re-enabling brings it back without disturbing the neighbours', async () => {
    const response = await post('/dsh-pluginhub/toggle', { name: CROSS, enabled: true })
    expect(response.status).toBe(200)
    await settle()
    await scaffold.restart()
    expect(reallyLive(CROSS)).toBe(true)
  ...
  it('a client that vanishes mid-uninstall still gets a clean removal (#163)', async () => {
    // Reported as: leave the pluginhub page while "uninstalling…" is showing,
    ...
    const abandoned = fetch(`${base}/dsh-pluginhub/uninstall`, {
      method: 'POST',
      headers: { 'content-type': 'application/json', origin: base },
  ...
  it('uninstalls cleanly — dependency and bundle registration both gone', async () => {
    const response = await post('/dsh-pluginhub/uninstall', { name: A })
    expect(response.status).toBe(200)
    await settle()
    const current = await state()
    expect(Object.keys(current.installed)).not.toContain(A)
