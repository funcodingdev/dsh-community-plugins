...
import { pluginHubVersion } from '../../src/routes.ts'
import { dshAvailable, launchPluginHubScaffold } from './scaffold.ts'
...
describe.skipIf(!HAS_DSH)('web e2e: release channel', () => {
  ...
  beforeAll(async () => { scaffold = await launchPluginHubScaffold() }, 300_000)
  afterAll(async () => { await scaffold?.close() })

  const statePath = (): string => join(scaffold.home, 'profiles', 'web', '.dsh-pluginhub', 'state.json')

  const readState = (): Record<string, unknown> =>
    JSON.parse(readFileSync(statePath(), 'utf8')) as Record<string, unknown>

  /** POST as the pluginhub's own page does — the route requires a same origin. */
  ...
  const setChannel = async (channel: string): Promise<number> =>
    (await post('/dsh-pluginhub/channel', { channel })).status

  const channelNow = async (): Promise<unknown> => {
    const res = await fetch(`${scaffold.baseUrl}/dsh-pluginhub/status`, { cache: 'no-store' })
    return ((await res.json()) as { channel?: unknown }).channel
  }

  // The pack step builds the tarball from THIS checkout, in this same
  // process, so `pluginHubVersion()` reads the identical version that ends up
  ...
  const derived = resolveChannel(undefined, pluginHubVersion())
  const other = derived === 'stable' ? 'beta' : 'stable'

  it('records nothing until the user picks, and derives from the build', async () => {
    // A pluginhub that had written a default would be indistinguishable from
...
describe.skipIf(!HAS_DSH)('web e2e: the dev channel', () => {
  ...
  const statusNow = async (): Promise<any> =>
    await (await fetch(`${scaffold.baseUrl}/dsh-pluginhub/status`, { cache: 'no-store' })).json()
  ...
  it('is selected, written down, and survives a real restart', async () => {
    expect((await post('/dsh-pluginhub/channel', { channel: 'dev' })).status).toBe(200)
  ...
  it('refuses a channel that does not exist, without writing it', async () => {
    expect((await post('/dsh-pluginhub/channel', { channel: 'nightly' })).status).toBe(400)
    expect(readState().channel).toBe('dev')
  })
})
