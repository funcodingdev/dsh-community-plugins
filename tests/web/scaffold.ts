 ...
 * a throwaway DSH_HOME with the packed pluginhub installed, and hand the
 * caller a base url + console tripwire. Playwright is used as a library by
 * the specs; this file owns only the host side.
 *
 * The dsh CLI is resolved from DSH_PLUGINHUB_E2E_DSH (a full command line, e.g.
...
const DSH_CWD = process.env.DSH_PLUGINHUB_E2E_DSH_CWD ?? REPO_ROOT

/** The dsh launch command, or null when no dsh is reachable (specs skip). */
export function dshCommand(): string | null {
  const explicit = process.env.DSH_PLUGINHUB_E2E_DSH
 ...
 * asserted nothing. CI sets DSH_PLUGINHUB_E2E_REQUIRED=1 to make that loud.
 */
export function dshAvailable(): boolean {
  if (dshCommand() !== null) return true
  if (process.env.DSH_PLUGINHUB_E2E_REQUIRED === '1') {
    throw new Error(
      'DSH_PLUGINHUB_E2E_REQUIRED=1 but no dsh CLI is reachable — the e2e lane would have skipped every spec and passed green',
    )
  }
  return false
}
...
export interface WebScaffold {
   ...
   * URL directly: `openPluginHubPage` exchanges it outside browser navigation. */
 ...
 */
export async function openPluginHubPage(
  page: Page,
  ...
  try {
    ...
    const detail = error instanceof Error ? error.stack ?? error.message : String(error)
    throw new Error(`failed to open authenticated dsh pluginhub page:\n${redactAuthenticationSecrets(detail, exactSecrets)}`)
  }
}
...
export interface ScaffoldOptions {
   ...
   * npm registry and list in a curated catalog the pluginhub is pointed at.
 ...
 * a temp DSH_HOME with the pluginhub installed from the tarball.
 */
export async function launchPluginHubScaffold(options: ScaffoldOptions = {}): Promise<WebScaffold> {
  const command = dshCommand()
  if (command === null) throw new Error('no dsh available — set DSH_PLUGINHUB_E2E_DSH')
  const home = mkdtempSync(join(tmpdir(), 'dsph-e2e-home-'))
  let env: NodeJS.ProcessEnv = { ...process.env, DSH_HOME: home, CI: 'true' }

  // prepack builds lib/ + client and runs the preflight guard. The pluginhub's
  ...
  if (options.fixtures !== undefined && options.fixtures.length > 0) {
    ...
    env = { ...env, DSH_PLUGINHUB_REGISTRY_URL: registry.catalogUrl, npm_config_registry: registry.npmUrl }
  }

  const port = await freePort()
  const baseUrl = `http://127.0.0.1:${String(port)}`

  /** Spawn dsh and wait until the pluginhub answers, or explain why it never did.
  ...
  const boot = async (): Promise<{ child: ChildProcess; processLaunchUrl: string }> => {
    ...
    return await withFailureCleanup(async () => {
      ...
      for (;;) {
        ...
        if (!statusReady) {
          try {
            const res = await fetch(`${baseUrl}/dsh-pluginhub/status`, { signal: AbortSignal.timeout(2000) })
            statusReady = res.ok
          } catch { /* not up yet */ }
        }
  ...
  return {
     ...
     * to observe what the pluginhub's file-level work actually did: the profile
