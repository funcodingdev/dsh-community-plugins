...
import {
  activeRegion, asRegion, DEFAULT_NPM_REGISTRY, PLUGINHUB_CATALOG_URL, REGIONS, routesFor, setActiveRegion, throughProxy,
} from '../src/regions.ts'
import { codeloadAllowBuildsKey, codeloadTarball, githubCommitOfTarget, gitAllowBuildsKey, repoOfTarget } from '../src/sources.ts'
import { githubProxyInUse, githubUrl, setGithubProxy } from '../src/client/pluginhub-data.ts'
...
describe('routesFor', () => {
  it('leaves the global route on the official sources', () => {
    ...
    expect(routes.catalog).toEqual([{ kind: 'url', url: PLUGINHUB_CATALOG_URL }])
  })

  it('keeps the public catalog canonical while mirroring China package and GitHub traffic', () => {
  ...
  it('moves plugin package downloads without changing the GitHub catalog route', () => {
    const routes = routesFor('china', { DSH_PLUGINHUB_NPM_MIRROR: 'https://npm.internal' })
  ...
  it('lets the environment override each route', () => {
    const routes = routesFor('china', {
      DSH_PLUGINHUB_NPM_MIRROR: 'https://npm.internal/',
      DSH_PLUGINHUB_GITHUB_PROXY: 'https://gh.internal/',
    })
  ...
  it('replaces the whole source list when the catalog itself is overridden', () => {
    // Someone pointing the pluginhub at their own catalog does not want it
    ...
    const routes = routesFor('china', { DSH_PLUGINHUB_REGISTRY_URL: 'http://127.0.0.1:9/fixture.json' })
    expect(routes.catalog).toEqual([{ kind: 'url', url: 'http://127.0.0.1:9/fixture.json' }])
  })

  it('ignores blank overrides', () => {
    const routes = routesFor('china', { DSH_PLUGINHUB_NPM_MIRROR: '   ' })
