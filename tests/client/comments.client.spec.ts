/**
 * The three surfaces that show a plugin's comments — this pluginhub, the
 * github.com/funcodingdev/dsh-community-plugins detail page, and the dsh-plugin-hub catalog page — only
...
import { COMMENTS_ENABLED, GISCUS, commentsTerm, giscusLang, pluginSlug } from '../../src/client/comments.ts'
...
describe('giscus configuration', () => {
  it('matches the config the static site is built with', async () => {
    ...
    expect(COMMENTS_ENABLED).toBe(false)
    expect({
      repo: site.repo,
      repoId: site.repoId,
...
describe('pluginSlug', () => {
  ...
  it('agrees with the site builder for repository and monorepo URLs', () => {
    ...
    const plugins = [
      ...
      'https://github.com/another-owner/plugin',
      'https://github.com/owner/repo/tree/main/packages/a',
      'https://github.com/owner/repo/tree/main/packages/b',
      'https://github.com/owner/repo/tree/v1.2.3/examples/nested/plugin',
    ].map(url => ({ url }))
