...
/**
 * The pluginhub's card on the plugin configuration page.
 *
...
function stubFetch(options: {
  ...
  vi.stubGlobal('fetch', vi.fn((input: unknown, init?: RequestInit) => {
    ...
    if (path.endsWith('/dsh-pluginhub/status')) {
      return json({
    ...
    if (path.includes('/dsh-pluginhub/updates')) {
      return json({ updates: { 'dsh-community-plugins': options.channelSwitch !== undefined
    ...
    if (path.endsWith('/dsh-pluginhub/channel')) {
    ...
    if (path.endsWith('/dsh-pluginhub/region')) {
    ...
    if (path.endsWith('/dsh-pluginhub/self-uninstall')) {
      return options.removeOk === false
        ? json({ ok: false, error: options.error ?? 'boom' })
        : json({ ok: true, removed: 'dsh-community-plugins', restart: options.restart !== false })
    }
    return json({ ok: true })
  }))
}
...
describe('SettingsCard', () => {
  ...
  it('confirms before switching a locally packaged pluginhub to its matched online release', async () => {
    ...
    expect(calls.some(call => call.path.endsWith('/dsh-pluginhub/update'))).toBe(false)
    expect(screen.getByText(t('restoreHint'))).toBeTruthy()
    fireEvent.click(screen.getByRole('button', { name: t('restoreContinue') }))
    await waitFor(() => {
      expect(calls.find(call => call.path.endsWith('/dsh-pluginhub/update'))?.body)
        .toEqual({ name: 'dsh-community-plugins', restore: true })
    })
  })
  ...
  it('states the consequence of KEEPING the data, which is the surprising one', async () => {
    await open()
    fireEvent.click(screen.getByRole('button', { name: t('setSelfRemove') }))
    // Unchecked is the default, and its consequence — plugins the pluginhub
  ...
  it('offers no control in the end state that the server can no longer answer', async () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: t('setSelfRemoveConfirm') }))
    await waitFor(() => { expect(screen.getByText(t('setSelfRemoved'))).toBeTruthy() })
    // The package is gone from disk AND the pluginhub is out of the running
  ...
  it('asks exactly one thing — whether to keep the data', async () => {
    ...
    // (the restart route dies with the pluginhub), not because the answer was
  ...
  it('retires the pluginhub\'s own menu entry once the package is gone', async () => {
    // The visible half of the removal. Without it the left menu still offers
    // "社区插件", and clicking it opens a page whose server routes have just
  ...
  it('leaves the menu alone when removal fails', async () => {
    ...
    // The pluginhub is still installed and still working; removing its menu
...
describe('SettingsCard — channels', () => {
  ...
  it('sends the channel the user picked', async () => {
    ...
    await waitFor(() => {
      expect(calls.find(call => call.path.includes('/dsh-pluginhub/channel'))?.body).toEqual({ channel: 'dev' })
    })
  })
...
describe('SettingsCard — download region', () => {
  ...
  it('sends the region the user picked', async () => {
    ...
    await waitFor(() => {
      expect(calls.find(call => call.path.endsWith('/dsh-pluginhub/region'))?.body).toEqual({ region: 'china' })
    })
  })
  ...
  it('re-reads the resolved proxy from the server instead of deriving it', async () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: t('setRegionChina') }))
    await waitFor(() => {
      expect(calls.filter(call => call.path.endsWith('/dsh-pluginhub/status'))).toHaveLength(1)
    })
  })
})
...
describe('SettingsCard — a channel switch is not an update', () => {
  it('names an older offer for what it is', async () => {
    ...
    // Never the update wording, and never the update button: the pluginhub
