...
describe('catalogFromPackage', () => {
  ...
  function stub(options: {
    ...
    const tarballUrl = `${options.tarballHost ?? REGISTRY}/dsh-plugin-hub/-/dsh-plugin-hub-${version}.tgz`
  ...
  it('reads the catalog and reports the version it came from', async () => {
    stub()
    await expect(catalogFromPackage(REGISTRY, 'dsh-plugin-hub')).resolves
  ...
  it('follows dist.tarball onto the mirror instead of composing an origin URL', async () => {
    ...
    await catalogFromPackage(REGISTRY, 'dsh-plugin-hub')
  ...
  it('skips the download when the published version is the one already held', async () => {
    ...
    const result = await catalogFromPackage(REGISTRY, 'dsh-plugin-hub', '2026.8.22')
  ...
  for (const [label, options] of [
    ...
    it(`throws so the caller can fall back when ${label}`, async () => {
      stub(options)
      await expect(catalogFromPackage(REGISTRY, 'dsh-plugin-hub')).rejects.toThrow()
    })
  }
  ...
  it('throws when the metadata names no tarball at all', async () => {
    ...
    await expect(catalogFromPackage(REGISTRY, 'dsh-plugin-hub')).rejects.toThrow(/no version or tarball/)
  })
})
