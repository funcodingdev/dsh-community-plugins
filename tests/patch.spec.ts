...
function patchDir(): string {
  return mkdtempSync(join(tmpdir(), 'dsph-patch-'))
}
      ...
      const host: PatchHost = {
        loader: {
          entries: () => [
            ...
            // PluginHub-owned namespaces must never map to user-patch rows.
            { options: { id: 'pluginhub-loop-main', name: 'dsh-loop' } },
...
describe('carrierDisableIds', () => {
  ...
  it('ignores a config-only neighbour tweak so its re-enable keeps working (#147 e2e)', () => {
    ...
    try {
      ...
      installedBundle(dir, 'dsph-e2e-fixture-cross', [
        '- insert:',
        '    - id: dsph-fixture-cross',
        "      name: 'dsph-e2e-fixture-cross'",
        '- id: dsph-fixture-b',
        '  config:',
        '    tweakedByCross: true',
        '',
      ].join('\n'))
      expect(carrierDisableIds(dir, 'dsph-e2e-fixture-cross')).toEqual([])
    } finally {
      rmSync(dir, { recursive: true, force: true })
    }
  })
