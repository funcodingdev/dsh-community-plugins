/**
 * Persistent log sink: every pluginhub event survives the process, capped and
 * sanitized.
 */
...
import { configurePersistentLog, logEvent } from '../src/log.ts'

let home: string
let logFile: string
const readLines = (): string[] => readFileSync(logFile, 'utf8').split('\n').filter(line => line !== '')
beforeEach(() => {
  home = mkdtempSync(join(tmpdir(), 'dsph-log-'))
  logFile = join(home, '.dsh-pluginhub', 'log.ndjson')
})
afterEach(() => {
...
describe('persistent log sink', () => {
  ...
  it('trims an oversized file to its newest half on configure', () => {
    mkdirSync(join(home, '.dsh-pluginhub'), { recursive: true })
