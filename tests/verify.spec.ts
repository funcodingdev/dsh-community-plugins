...
let home: string
beforeEach(() => {
  home = mkdtempSync(join(tmpdir(), 'dsph-verify-'))
...
describe('verifyActivation (P0-2)', () => {
   ...
   * — the pluginhub said the install failed verification while the plugin was
...
describe('activationAfterReplace (post-update verdict)', () => {
  ...
  it('downgrades a plugin that was already running to "restart"', async () => {
    // Measured on a real host: updating the pluginhub 1.11.3 → 1.12.2 left the
...
describe('hasHostHalf (client-only updates need a refresh, not a restart)', () => {
  it('is false for a dsh.client package with no dsh.bundle', async () => {
    // Themes and skins. The pluginhub shim-mounts them so the loader has a live
    ...
    const dir = mkdtempSync(join(tmpdir(), 'dsph-hosthalf-'))
