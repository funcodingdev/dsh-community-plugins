...
describe('checkUpdates — github pins', () => {
  ...
  function profileWith(spec: string, commit: string | null, version = '1.0.0'): string {
    const dir = join(mkdtempSync(join(tmpdir(), 'dsph-upd-')), 'profiles', 'web')
  ...
  beforeEach(() => {
    home = mkdtempSync(join(tmpdir(), 'dsph-updhome-'))
...
describe('preferBeta (release channel)', () => {
  it('offers the prerelease only when it is actually newer', async () => {
    ...
    const answer = (beta: string | null) => {
      ...
      return versionOnChannel('dsh-pluginhub', 'beta', '1.14.0')
    }
  ...
  it('falls back to stable when the beta tag cannot be read', async () => {
    ...
    await expect(versionOnChannel('dsh-pluginhub', 'beta', '1.14.0')).resolves.toBe('1.14.0')
    // ...and with nothing on either side it stays honest about knowing nothing.
    await expect(versionOnChannel('dsh-pluginhub', 'beta', null)).resolves.toBeNull()
  })
  ...
  it('the stable channel is exactly latest, which is what makes it leavable', async () => {
    ...
    // points at — and THAT is the difference the pluginhub can act on. Reading
    ...
    await expect(versionOnChannel('dsh-pluginhub', 'stable', '1.13.1')).resolves.toBe('1.13.1')
  ...
  it('the dev channel takes the newest of latest, beta and dev', async () => {
    ...
    await expect(versionOnChannel('dsh-pluginhub', 'dev', '1.13.1')).resolves.toBe('1.15.0-dev.20260818-abc1234')
    ...
    await expect(versionOnChannel('dsh-pluginhub', 'dev', '1.13.1')).resolves.toBe('1.14.0-beta.9')
  })
})
...
describe('checkUpdates — the channel is part of the cache key', () => {
  it('re-resolves when the beta opt-in changes, without waiting out the TTL', async () => {
    ...
    // writes PluginHubSettings directly, and `onChange` updates the resolved
    // config in place. Keyed on the profile alone, the pluginhub would keep
    ...
    const dir = join(mkdtempSync(join(tmpdir(), 'dsph-chan-')), 'profiles', 'web')
    mkdirSync(join(dir, 'node_modules', 'dsh-pluginhub'), { recursive: true })
    writeFileSync(join(dir, 'package.json'), JSON.stringify({ dependencies: { 'dsh-pluginhub': '^1.0.0' } }))
    writeFileSync(join(dir, 'node_modules', 'dsh-pluginhub', 'package.json'), JSON.stringify({ name: 'dsh-pluginhub', version: '1.0.0' }))
    ...
    expect(stable['dsh-pluginhub']?.latest).toBe('1.5.0')
    expect(asked.some(url => url.endsWith('/beta'))).toBe(false)

    asked.length = 0
    const beta = await checkUpdates('web', false, dir, new Map([['dsh-pluginhub', 'beta' as const]]))
    expect(asked.some(url => url.endsWith('/beta')), 'served the cached stable answer to a beta subscriber').toBe(true)
    expect(beta['dsh-pluginhub']?.latest).toBe('2.0.0-beta.1')
...
describe('updateAvailable means NEWER, and only that', () => {
  const bed = (installedVersion: string, tags: Record<string, string>) => {
    const dir = join(mkdtempSync(join(tmpdir(), 'dsph-dir-')), 'profiles', 'web')
    mkdirSync(join(dir, 'node_modules', 'dsh-pluginhub'), { recursive: true })
    writeFileSync(join(dir, 'package.json'), JSON.stringify({ dependencies: { 'dsh-pluginhub': '^1.0.0' } }))
    writeFileSync(join(dir, 'node_modules', 'dsh-pluginhub', 'package.json'), JSON.stringify({ name: 'dsh-pluginhub', version: installedVersion }))
    vi.stubGlobal('fetch', vi.fn((url: unknown) => {
      const tag = String(url).split('/').pop() ?? ''
  ...
  it('reports a backwards move as a channel switch, never as an update', async () => {
    ...
    // pluginhub page reads that flag in three places it was never taught about
    ...
    try {
      const row = (await checkUpdates('web', true, dir, new Map([['dsh-pluginhub', 'stable' as const]])))['dsh-pluginhub']
  ...
  it('offers no switch when the channel already points at what is installed', async () => {
    ...
    try {
      const row = (await checkUpdates('web', true, dir, new Map([['dsh-pluginhub', 'beta' as const]])))['dsh-pluginhub']
  ...
  it('never offers a switch for a package that does not follow a channel', async () => {
    // Only the pluginhub follows one. An ordinary plugin whose `latest` went
    // backwards is #64's case, and its answer is to refuse, not to offer.
    const dir = bed('2.0.0', { latest: '1.0.0' })
    try {
      const row = (await checkUpdates('web', true, dir))['dsh-pluginhub']
      expect(row?.updateAvailable).toBe(false)
      expect(row?.channelSwitch).toBeUndefined()
    } finally { vi.unstubAllGlobals(); rmSync(dir, { recursive: true, force: true }) }
  })
